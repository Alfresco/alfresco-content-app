...
export class AppExtensionService implements RuleContext {
    ...
    setup(config: ExtensionConfig) {
        ...
        this.documentListPresets = {
            ...
            searchLibraries: this.getDocumentListPreset(
                config,
                'search-libraries'
            ),
        };
    ...
    protected loadNavBar(config: ExtensionConfig): Array<NavBarGroupRef> {
        return this.loader.getElements<NavBarGroupRef>(
            config,
            'features.navbar'
        );
    }
                ...
                return {
                    ...
                    items: (group.items || [])
                        ...
                        .map((item) => {
                            ...
                            if (!item.click) {
                                const routeRef = this.extensions.getRouteById(
                                    item.route
                                );
                                const url = `/${
                                    routeRef ? routeRef.path : item.route
                                }`;
                                return {
                                    ...item,
                                    url,
    ...
    filterDisabled(
        object: Array<{ disabled: boolean }> | { disabled: boolean }
    ) {
    ...
    getApplicationRoutes(): Array<Route> {
        return this.extensions.routes.map((route) => {
            ...
            return {
                path: route.path,
                component: this.getComponentById(
                ...
                if (action.type === ContentActionType.menu) {
                    ...
                    if (copy.children && copy.children.length > 0) {
                        copy.children = copy.children
                            .filter((entry) => !entry.disabled)
                            .filter((childAction) =>
                                this.filterVisible(childAction)
                            )
                            .sort(sortByOrder)
                            .reduce(reduceSeparators, []);
                    }
    ...
    isViewerExtensionDisabled(extension: any): boolean {
        if (extension) {
            ...
            if (extension.rules && extension.rules.disabled) {
                return this.extensions.evaluateRule(
