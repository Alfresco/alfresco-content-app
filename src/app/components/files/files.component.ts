...
export class FilesComponent extends PageComponent implements OnInit, OnDestroy {
    ...
    onFileUploadedEvent(event: FileUploadEvent) {
        ...
        if (
            node &&
            node.entry &&
            node.entry.parentId === this.getParentNodeId()
        ) {
            ...
            if (this.nodePath) {
                ...
                if (correspondingNodePath) {
                    const correspondingIndex =
                        this.nodePath.length -
                        this.nodePath.indexOf(correspondingNodePath);
                    this.displayFolderParent(
                        event.file.options.path,
                        correspondingIndex
                    );
                }
            }
    ...
    displayFolderParent(filePath = '', index: number) {
        ...
        const alreadyDisplayedParentFolder = currentFoldersDisplayed.find(
            (row) =>
                row.node.entry.isFolder && row.node.entry.name === parentName
        );
