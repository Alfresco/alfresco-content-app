...
export class AppViewerComponent implements OnInit, OnDestroy {
    ...
    onNavigateBefore(event: MouseEvent | KeyboardEvent): void {
        ...
        this.store.dispatch(
            new ViewNodeAction(this.previousNodeId, { location })
        );
    }
    ...
    async getNearestNodes(
        ...
        if (nodeId && folderId) {
            try {
                const ids = await this.getFileIds(
                    this.navigateSource,
                    folderId
                );
    ...
    async getFileIds(source: string, folderId?: string): Promise<string[]> {
        ...
        if (source === 'favorites') {
            ...
            const files = nodes.list.entries.map(
                (obj) => obj.entry.target.file
            );
        ...
        if (source === 'recent-files') {
            ...
            const sortingKey =
                ...
                'modifiedAt';
            const sortingDirection =
                this.preferences.get('recent-files.sorting.direction') ||
                'desc';
