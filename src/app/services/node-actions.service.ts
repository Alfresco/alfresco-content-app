...
export class NodeActionsService {
            ...
            destinationSelection.subscribe(
                (selections: MinimalNodeEntryEntity[]) => {
                    ...
                    const contentEntryId =
                        contentEntry.nodeId || contentEntry.id;
        ...
        if (contentEntities && contentEntities.length) {
            const nonEntryNode = contentEntities.find(
                (node) =>
                    !node ||
                    !node.entry ||
                    !(node.entry.nodeId || node.entry.id)
            );
            return !nonEntryNode;
        }
        return false;
    ...
    private normalizeSitePath(node: MinimalNodeEntryEntity) {
        ...
        if (this.isSiteContainer(node)) {
            ...
            const docLib = elements.findIndex(
                (el) => el.name === 'documentLibrary'
            );
            if (docLib > -1) {
                        ...
                        return this.getChildByName(
                            ...
                            mergeMap((folderOnInitialLocation) => {
                                if (folderOnInitialLocation) {
                                    // Check if there's nodeId for Shared Files
                                    const nodeEntryId =
                                        nodeEntry.nodeId || nodeEntry.id;
                                    // delete it from location
                                    return this.contentApi
                                        .deleteNode(nodeEntryId)
                                        .pipe(
                                            mergeMap(() => {
                                                this.moveDeletedEntries.push(
                                                    nodeEntry
                                                );
                                                return of(newContent);
                                            })
    ...
    processResponse(data: any): any {
        ...
        if (Array.isArray(data)) {
            return data.reduce((acc, next) => {
                if (next instanceof Error) {
                    ...
                    const foundEntry = folderMoveResponseData.find(
                        (node) =>
                            (node.itemMoved && node.itemMoved.entry) ||
                            (node && node.entry)
                    );
            ...
            if (
                (data.itemMoved && data.itemMoved.entry) ||
                (data && data.entry)
            ) {
