...
describe('ContentManagementService', () => {
    ...
    describe('Copy node action', () => {
        ...
        it('notifies generic error message on all errors, but 403', () => {
            spyOn(nodeActions, 'copyNodes').and.returnValue(
                throwError(
                    new Error(JSON.stringify({ error: { statusCode: 404 } }))
                )
            );
            ...
            const selection: any[] = [
                { entry: { id: 'node-to-copy-1', name: 'name1' } },
                {
                    entry: {
                        id: 'node-to-copy-2',
                        name:
                            'folder-with-name-already-existing-on-destination',
                    },
                },
            ];
    ...
    describe('Move node action', () => {
        ...
        it('notifies error if success message was not emitted', () => {
            const nodes: any[] = [
                { entry: { id: 'node-to-move-id', name: 'name' } },
            ];
            const moveResponse = {
                succeeded: [],
                failed: [],
        ...
        it('notifies conflict error message on 409', () => {
            spyOn(nodeActions, 'moveNodes').and.returnValue(
                throwError(
                    new Error(JSON.stringify({ error: { statusCode: 409 } }))
                )
            );
    ...
    describe('Undo Move action', () => {
        ...
        it('should move node back to initial parent, after succeeded move', () => {
            ...
            const movedItems = {
                ...
                partiallySucceeded: [],
                succeeded: [
                    { itemMoved: node, initialParentId: initialParent },
                ],
            };
        ...
        it('should notify when error occurs on Undo Move action', fakeAsync((
            ...
            const movedItems = {
                ...
                partiallySucceeded: [],
                succeeded: [
                    { itemMoved: childMoved, initialParentId: initialParent },
                ],
            };
    ...
    describe('Delete action', () => {
        it('should raise info message on successful single file deletion', fakeAsync((
            done
        ) => {
            spyOn(contentApi, 'deleteNode').and.returnValue(of(null));
        ...
        it('should raise error message on failed single file deletion', fakeAsync((
            done
        ) => {
            spyOn(contentApi, 'deleteNode').and.returnValue(throwError(null));
        ...
        it('should raise info message on successful multiple files deletion', fakeAsync((
            done
        ) => {
            spyOn(contentApi, 'deleteNode').and.returnValue(of(null));
        ...
        it('should raise error message failed multiple files deletion', fakeAsync((
            done
        ) => {
            spyOn(contentApi, 'deleteNode').and.returnValue(throwError(null));
        ...
        it('should raise warning message when only one file is successful', fakeAsync((
            done
        ) => {
        ...
        it('should raise warning message when some files are successfully deleted', fakeAsync((
            done
        ) => {
            spyOn(contentApi, 'deleteNode').and.callFake((id) => {
                if (id === '1') {
    ...
    describe('Permanent Delete', () => {
        ...
        describe('notification', () => {
            it('raises warning on multiple fail and one success', fakeAsync((
                done
            ) => {
                actions$.pipe(
                    ofType<SnackbarWarningAction>(SnackbarActionTypes.Warning),
    ...
    describe('Restore Deleted', () => {
        ...
        it('should navigate to library folder when node is a library content', fakeAsync(() => {
            ...
            expect(
                store.dispatch['calls'].argsFor(1)[0].userAction
                    .action instanceof NavigateToParentFolder
            ).toBe(true);
        }));
        ...
        describe('notification', () => {
            ...
            it('should raise error message when restored node exist, error 409', fakeAsync((
                done
            ) => {
            ...
            it('should raise error message when restored node returns different statusCode', fakeAsync((
                done
            ) => {
            ...
            it('should raise error message when restored node location is missing', fakeAsync((
                done
            ) => {
                const error = { message: '{ "error": { } }' };
            ...
            it('should raise info message when restore multiple nodes', fakeAsync((
                done
            ) => {
                spyOn(contentApi, 'restoreNode').and.callFake((id) => {
                    if (id === '1') {
    ...
    describe('Share Node', () => {
        ...
        it('should emit event when node is un-shared', fakeAsync(() => {
            const node: any = { entry: { id: '1', name: 'name1' } };
            spyOn(
                contentManagementService.linksUnshared,
                'next'
            ).and.callThrough();
            spyOn(dialog, 'open').and.returnValue({
                afterClosed: () => of(node),
    ...
    describe('Unlock Node', () => {
        ...
        it('should raise error when unlock node fails', fakeAsync((done) => {
            ...
            spyOn(store, 'dispatch').and.callThrough();
            store.dispatch(
                new UnlockWriteAction({
                    entry: { id: 'node-id', name: 'some-file' },
                })
            );
            tick();
            flush();
    ...
    describe('editFolder', () => {
        it('should open dialog with FolderDialogComponent instance', () => {
            ...
            const node: any = {
                entry: { id: '1', name: 'name1', isFolder: true },
            };
            spyOn(dialog, 'open').and.returnValue(mockDialogInstance);
        ...
        it('should raise error when edit operation fails', fakeAsync(() => {
            ...
            mockDialogInstance.componentInstance.error.next(
                'edit folder error'
            );
        ...
        it('should call nodeUpdated event with edited node data', fakeAsync(() => {
            ...
            expect(alfrescoApiService.nodeUpdated.next).toHaveBeenCalledWith(
                newNode
            );
        }));
    });
