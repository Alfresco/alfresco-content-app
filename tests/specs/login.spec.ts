...
describe('Login', () => {
    ...
    describe('general tests', () => {
        it('login page layout - [C213089]', async () => {
            ...
            expect(username).toBe(true, 'username input is not enabled');
            expect(password).toBe(true, 'password input is not enabled');
            expect(submit).toBe(false, 'SIGN IN button is enabled');

            const passwordType = await page.$eval('#password', el => el.getAttribute('type'));
            expect(passwordType).toBe('password', 'Password is not hidden by default');
        });

        it('change password visibility - [C213091]', async () => {
            await page.type('#password', 'any-password');

            let passwordType = await page.$eval('#password', el => el.getAttribute('type'));
            expect(passwordType).toBe('password');

            await page.click('.adf-login-password-icon');

            passwordType = await page.$eval('#password', el => el.getAttribute('type'));
            expect(passwordType).toBe('text', 'password is not visible');
        });
    });
    ...
    describe('with valid credentials', () => {
        ...
        it(`displays user's name in header - [C213108]`, async () => {
            ...
            const fullName = await page.$eval('.current-user__full-name', (el: any) => el.innerText);
        ...
        it('redirects to Personal Files when pressing browser Back while already logged in - [C213109]', async () => {
            ...
            expect(url).toContain('/personal-files', 'user not logged in');
    ...
    describe('with invalid credentials', () => {
        ...
        it('shows error when entering nonexistent user - [C213093]', async () => {
            ...
            const err = await page.$eval('.login-error-message', (el: any) => el.innerText);
