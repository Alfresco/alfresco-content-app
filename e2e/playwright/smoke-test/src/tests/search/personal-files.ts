...
export function personalFilesTests(userName: string, parentName: string) {
  test.describe('Pagination controls : ', () => {
    ...
    test('[XAT-4592] Pagination control default values', async ({ personalFiles }) => {
    ...
    test('[XAT-4593] Items per page values', async ({ personalFiles }) => {
    ...
    test('[XAT-4595] Change the current page from the page selector', async ({ personalFiles }) => {
    ...
    test('[XAT-4598] Next and Previous buttons navigation', async ({ personalFiles }) => {
      await personalFiles.pagination.openMaxItemsMenu();
      await personalFiles.pagination.clickMenuItem('25');
      expect(await personalFiles.pagination.getMaxItems()).toContain('25');
      await personalFiles.pagination.clickOnNextPage();
    ...
    test('[XAT-4596] Previous button is disabled on first page', async ({ personalFiles }) => {
      expect(await personalFiles.pagination.getCurrentPage()).toContain('Page 1');
      expect(await personalFiles.pagination.isPreviousEnabled()).toBe(false);
    });

    test('[XAT-4597] Next button is disabled on last page', async ({ personalFiles }) => {
      await personalFiles.pagination.openMaxItemsMenu();
