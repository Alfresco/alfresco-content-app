...
export class SitesApi extends RepoApi {
    ...
    async getSite(siteId: string) {
        try {
            await this.apiAuth();
            return this.sitesApi.getSite(siteId);
        } catch (error) {
            this.handleError(
                `${this.constructor.name} ${this.getSite.name}`,
                error
            );
            return null;
        }
    ...
    async getSites() {
        try {
            await this.apiAuth();
            return this.sitesApi.listSiteMembershipsForPerson(
                this.getUsername()
            );
        } catch (error) {
            this.handleError(
                `${this.constructor.name} ${this.getSites.name}`,
                error
            );
            return null;
        }
    ...
    async getDocLibId(siteId: string) {
        try {
            await this.apiAuth();
            return (await this.sitesApi.listSiteContainers(siteId)).list
                .entries[0].entry.id;
        } catch (error) {
            this.handleError(
    ...
    async getTitle(siteId: string) {
        try {
            ...
            this.handleError(
                `${this.constructor.name} ${this.getTitle.name}`,
                error
            );
            return null;
        }
    ...
    async deleteSite(siteId: string, permanent: boolean = true) {
        try {
            await this.apiAuth();
            return this.sitesApi.deleteSite(siteId, { permanent });
        } catch (error) {
            this.handleError(
    ...
    async updateSiteMember(siteId: string, userId: string, role: string) {
        ...
        try {
            await this.apiAuth();
            return this.sitesApi.updateSiteMembership(siteId, userId, siteRole);
        } catch (error) {
            this.handleError(
    ...
    async addSiteMember(siteId: string, userId: string, role: string) {
        ...
        try {
            await this.apiAuth();
            return this.sitesApi.createSiteMembership(siteId, memberBody);
        } catch (error) {
            this.handleError(
    ...
    async requestToJoin(
        ...
        try {
            await this.apiAuth();
            return this.sitesApi.createSiteMembershipRequestForPerson(
                '-me-',
                body
            );
        } catch (error) {
    ...
    async waitForApi(data: { expect: number }) {
        try {
            const sites = async () => {
                const totalItems = (await this.getSites()).list.pagination
                    .totalItems;
