...
export class NodesApi extends RepoApi {
    ...
    async getNodeByPath(relativePath: string = '/'): Promise<NodeEntry | null> {
        try {
            await this.apiAuth();
            return this.nodesApi.getNode('-my-', { relativePath });
        } catch (error) {
            this.handleError(
    ...
    async getNodeIdFromParent(name: string, parentId: string): Promise<string> {
        try {
            const children = (await this.getNodeChildren(parentId)).list
                .entries;
            return (
                children.find((elem) => elem.entry.name === name).entry.id || ''
            );
        } catch (error) {
            this.handleError(
    ...
    async getNodeDescription(name: string, parentId: string): Promise<string> {
        try {
            const children = (await this.getNodeChildren(parentId)).list
                .entries;
            return (
                children.find((elem) => elem.entry.name === name).entry
    ...
    async getNodeProperty(nodeId: string, property: string): Promise<string> {
        try {
            ...
            return (
                (node.entry.properties && node.entry.properties[property]) || ''
            );
        } catch (error) {
            this.handleError(
    ...
    async getSharedId(nodeId: string): Promise<string> {
        try {
            const sharedId = await this.getNodeProperty(
                nodeId,
                'qshare:sharedId'
            );
            return sharedId || '';
    ...
    async deleteNodeChildren(
        ...
        try {
            const listEntries = (await this.getNodeChildren(parentId)).list
                .entries;
            let nodeIds: string[];
            if (exceptNodesNamed) {
                nodeIds = listEntries
                    .filter(
                        (entries) =>
                            !exceptNodesNamed.includes(entries.entry.name)
                    )
    ...
    async createNode(
        ...
        if (imageProps) {
            nodeBody.properties = Object.assign(
                nodeBody.properties,
                imageProps
            );
        }
        ...
        try {
            await this.apiAuth();
            return this.nodesApi.createNode(parentId, nodeBody, {
                majorVersion,
            });
        } catch (error) {
            this.handleError(
    ...
    async createImage(
        ...
        try {
            return this.createImageNode(name, parentId, title, description);
        } catch (error) {
            this.handleError(
    ...
    async createChildren(data: NodeBodyCreate[]): Promise<NodeEntry | any> {
        try {
            await this.apiAuth();
            return this.nodesApi.createNode('-my-', data as any);
        } catch (error) {
            this.handleError(
    ...
    async createFileLink(
        ...
        try {
            await this.apiAuth();
            const link = await this.nodesApi.createNode(
                destinationId,
                nodeBody
            );
    ...
    async editNodeContent(
        ...
        try {
            await this.apiAuth();
            return this.nodesApi.updateNodeContent(nodeId, content);
        } catch (error) {
            this.handleError(
    ...
    async setInheritPermissions(
        ...
        try {
            await this.apiAuth();
            return this.nodesApi.updateNode(nodeId, data);
        } catch (error) {
            this.handleError(
    ...
    async lockFile(
        ...
        try {
            await this.apiAuth();
            return this.nodesApi.lockNode(nodeId, data);
        } catch (error) {
            this.handleError(
                `${this.constructor.name} ${this.lockFile.name}`,
                error
            );
            return null;
        }
    ...
    async getLockOwner(nodeId: string): Promise<string> {
        try {
            const lockOwner = await this.getNodeProperty(
                nodeId,
                'cm:lockOwner'
            );
            return lockOwner || '';
    ...
    async isFileLockedByName(
        ...
        try {
            ...
            return this.isFileLockedWrite(id);
        } catch (error) {
            this.handleError(
