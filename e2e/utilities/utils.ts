...
export class Utils {
    ...
    static async waitUntilElementClickable(
        element: ElementFinder
    ): Promise<void> {
        await browser
            .wait(EC.elementToBeClickable(element), BROWSER_WAIT_TIMEOUT)
            .catch(Error);
    }

    static async typeInField(
        elem: ElementFinder,
        value: string
    ): Promise<void> {
    ...
    static async uploadFileNewVersion(fileFromOS: string): Promise<void> {
        ...
        await el.sendKeys(
            `${E2E_ROOT_PATH}/resources/test-files/${fileFromOS}`
        );
    }
}
