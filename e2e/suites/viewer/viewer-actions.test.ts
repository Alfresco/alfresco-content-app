...
describe('Viewer actions', () => {
    ...
    describe('from Personal Files', () => {
        ...
        beforeAll(async (done) => {
            ...
            destinationId = (await apis.user.nodes.createFolder(destination))
                .entry.id;

            docxFileId = (
                await apis.user.upload.uploadFileWithRename(
                    docxFile,
                    parentId,
                    docxPersonalFiles
                )
        ...
        it('[C268129] - Download action', async () => {
            ...
            expect(await Utils.fileExistsOnOS(docxPersonalFiles)).toBe(
                true,
        ...
        it('[C268130] - Copy action', async () => {
            ...
            expect(await copyMoveDialog.isDialogOpen()).toBe(
                true,
                'Dialog is not open'
            );
            ...
            expect(await dataTable.isItemPresent(docxPersonalFiles)).toBe(
                true,
            ...
            expect(await dataTable.isItemPresent(docxPersonalFiles)).toBe(
                true,
                'Item is not present in destination'
            );
        ...
        it('[C268131] - Move action', async () => {
            ...
            await viewer.clickClose();
            expect(await dataTable.isItemPresent(xlsxPersonalFiles)).toBe(
                false,
                'Item was not moved'
            );
            await page.clickPersonalFilesAndWait();
        ...
        it('[C268132] - Favorite action', async () => {
            ...
            expect(await apis.user.favorites.isFavorite(docxFileId)).toBe(
                true,
                'Item is not favorite'
            );
            expect(await dataTable.isItemPresent(docxPersonalFiles)).toBe(
                true,
                'Item is not present in Favorites list'
            );
        });
        ...
        it('[C268133] - Delete action', async () => {
            ...
            expect(await page.getSnackBarMessage()).toContain(
                `${pdfPersonalFiles} deleted`
            );
            expect(await viewer.isViewerOpened()).toBe(
                false,
                'Viewer is opened'
            );
            await Utils.pressEscape();
            await page.clickTrashAndWait();
            expect(await dataTable.isItemPresent(pdfPersonalFiles)).toBe(
                true,
                'Item is not present in Trash'
            );
        });
        ...
        it('[C297584] - Edit Offline action', async () => {
            ...
            expect(await Utils.fileExistsOnOS(fileForEditOffline)).toBe(
                true,
                'File not found in download location'
            );
            expect(
                await apis.user.nodes.isFileLockedWrite(fileForEditOfflineId)
            ).toBe(true, `${fileForEditOffline} is not locked`);
            expect(await viewer.isViewerOpened()).toBe(
                true,
                'Viewer is not open'
            );
        ...
        it('[C297585] - Cancel Editing action', async () => {
            ...
            expect(
                await apis.user.nodes.isFileLockedWriteWithRetry(
                    fileForCancelEditingId,
                    false
                )
            ).toBe(false, `${fileForCancelEditing} is still locked`);
            expect(await viewer.isViewerOpened()).toBe(
                true,
                'Viewer is not open'
            );
        ...
        it('[C297586] - Upload new version action', async () => {
            ...
            expect(
                await apis.user.nodes.getFileVersionType(filePersonalFilesId)
            ).toEqual('MAJOR', 'File has incorrect version type');
            expect(
                await apis.user.nodes.getFileVersionLabel(filePersonalFilesId)
        ...
        it('[MNT-21058] - Upload new version action when node is locked', async () => {
            ...
            expect(await toolbar.menu.isCancelEditingPresent()).toBe(
                true,
                `'Cancel Editing' button should be shown`
            );
            expect(await toolbar.menu.isEditOfflinePresent()).toBe(
                false,
        ...
        it(' [C279282] - Full screen action', async () => {
            ...
            expect(await viewer.isViewerOpened()).toBe(
                true,
                'Viewer is closed after pressing Full screen'
            );
        ...
        it('[C286313] - Share action', async () => {
            ...
            expect(await shareDialog.isDialogOpen()).toBe(
                true,
                'Dialog is not open'
            );
        ...
        it('[C286316] - Manage Versions action', async () => {
            ...
            expect(await manageVersionsDialog.isDialogOpen()).toBe(
                true,
                'Dialog is not open'
            );
    ...
    describe('from File Libraries', () => {
        ...
        it('[C286369] - Download action', async () => {
            ...
            await toolbar.clickDownload();
            expect(await Utils.fileExistsOnOS(docxLibraries)).toBe(
                true,
                'File not found in download location'
            );
        });
        ...
        it('[C286371] - Move action', async () => {
            ...
            expect(await page.getSnackBarMessage()).toContain('Moved 1 item');
            await viewer.clickClose();
            expect(await dataTable.isItemPresent(xlsxLibraries)).toBe(
                false,
                'Item was not moved'
            );
            await page.clickPersonalFilesAndWait();
        ...
        it('[C286373] - Delete action', async () => {
            ...
            await Utils.pressEscape();
            await page.clickTrashAndWait();
            expect(await dataTable.isItemPresent(pdfLibraries)).toBe(
                true,
                'Item is not present in Trash'
            );
        });
        ...
        it('[C297591] - Upload new version action', async () => {
            ...
            expect(await viewer.getFileTitle()).toContain(docxFile2);
            expect(
                await apis.user.nodes.getFileVersionType(fileLibrariesId)
            ).toEqual('MAJOR', 'File has incorrect version type');
            expect(
                await apis.user.nodes.getFileVersionLabel(fileLibrariesId)
            ).toEqual('2.0', 'File has incorrect version label');
        });
    ...
    describe('from Recent Files', () => {
        ...
        it('[C286383] - Download action', async () => {
            ...
            await toolbar.clickDownload();
            expect(await Utils.fileExistsOnOS(docxRecentFiles)).toBe(
                true,
                'File not found in download location'
            );
        });
        ...
        it(' [C286384] - Copy action', async (done) => {
            ...
            expect(await page.getSnackBarMessage()).toContain('Copied 1 item');
            await viewer.clickClose();
            expect(await dataTable.isItemPresent(docxRecentFiles)).toBe(
                true,
                'Item is not in the list'
            );
            await page.clickPersonalFilesAndWait();
        ...
        it('[C286385] - Move action', async () => {
            ...
            expect(await page.getSnackBarMessage()).toContain('Moved 1 item');
            await viewer.clickClose();
            expect(await dataTable.isItemPresent(xlsxRecentFiles)).toBe(
                true,
                'Item is not in the list'
            );
            expect(
                await dataTable.getItemLocationTooltip(xlsxRecentFiles)
            ).toContain(destination, 'Item was not moved');
            await page.clickPersonalFilesAndWait();
            await dataTable.doubleClickOnRowByName(destination);
            expect(await dataTable.isItemPresent(xlsxRecentFiles)).toBe(
                true,
        ...
        it('[C286387] - Delete action', async () => {
            ...
            await Utils.pressEscape();
            await page.clickTrashAndWait();
            expect(await dataTable.isItemPresent(pdfRecentFiles)).toBe(
                true,
                'Item is not present in Trash'
            );
        });
        ...
        it('[C297596] - Upload new version action', async () => {
            ...
            expect(await viewer.getFileTitle()).toContain(docxFile2);
            expect(
                await apis.user.nodes.getFileVersionType(fileRecentId)
            ).toEqual('MAJOR', 'File has incorrect version type');
            expect(
                await apis.user.nodes.getFileVersionLabel(fileRecentId)
            ).toEqual('2.0', 'File has incorrect version label');
        });
    ...
    describe('from Shared Files', () => {
        ...
        it('[C286376] - Download action', async () => {
            ...
            await toolbar.clickDownload();
            expect(await Utils.fileExistsOnOS(docxSharedFiles)).toBe(
                true,
                'File not found in download location'
            );
        });
        ...
        it('[C286377] - Copy action', async () => {
            ...
            expect(await page.getSnackBarMessage()).toContain('Copied 1 item');
            await viewer.clickClose();
            expect(await dataTable.isItemPresent(docxSharedFiles)).toBe(
                true,
                'Item is not in the list'
            );
            await page.clickPersonalFilesAndWait();
        ...
        it('[C286378] - Move action', async () => {
            ...
            expect(await page.getSnackBarMessage()).toContain('Moved 1 item');
            await viewer.clickClose();
            expect(await dataTable.isItemPresent(xlsxSharedFiles)).toBe(
                true,
                'Item is not in the list'
            );
            expect(
                await dataTable.getItemLocationTooltip(xlsxSharedFiles)
            ).toContain(destination, 'Item was not moved');
            await page.clickPersonalFilesAndWait();
            await dataTable.doubleClickOnRowByName(destination);
            expect(await dataTable.isItemPresent(xlsxSharedFiles)).toBe(
        ...
        it('[C286380] - Delete action', async () => {
            ...
            await Utils.pressEscape();
            await page.clickTrashAndWait();
            expect(await dataTable.isItemPresent(pdfSharedFiles)).toBe(
                true,
                'Item is not present in Trash'
            );
        });
        ...
        it('[C297603] - Upload new version action', async () => {
            ...
            expect(await viewer.getFileTitle()).toContain(docxFile2);
            expect(
                await apis.user.nodes.getFileVersionType(fileSharedId)
            ).toEqual('MAJOR', 'File has incorrect version type');
            expect(
                await apis.user.nodes.getFileVersionLabel(fileSharedId)
            ).toEqual('2.0', 'File has incorrect version label');
        });
    ...
    describe('from Favorites', () => {
        ...
        beforeAll(async (done) => {
            ...
            fileFavId = (await apis.user.upload.uploadFile(docxFile2, parentId))
                .entry.id;
        ...
        it('[C286390] - Download action', async () => {
            ...
            await toolbar.clickDownload();
            expect(await Utils.fileExistsOnOS(docxFavorites)).toBe(
                true,
                'File not found in download location'
            );
        });
        ...
        it('[C286391] - Copy action', async (done) => {
            ...
            expect(await page.getSnackBarMessage()).toContain('Copied 1 item');
            await viewer.clickClose();
            expect(await dataTable.isItemPresent(docxFavorites)).toBe(
                true,
                'Item is not in the list'
            );
            await page.clickPersonalFilesAndWait();
        ...
        it('[C286392] - Move action', async () => {
            ...
            expect(await page.getSnackBarMessage()).toContain('Moved 1 item');
            await viewer.clickClose();
            expect(await dataTable.isItemPresent(xlsxFavorites)).toBe(
                true,
                'Item is not in the list'
            );
            expect(
                await dataTable.getItemLocationTooltip(xlsxFavorites)
            ).toContain(destination, 'Item was not moved');
            await page.clickPersonalFilesAndWait();
            await dataTable.doubleClickOnRowByName(destination);
            expect(await dataTable.isItemPresent(xlsxFavorites)).toBe(
        ...
        it('[C286393] - Favorite action', async () => {
            ...
            await viewer.clickClose();
            await page.clickFavoritesAndWait();
            expect(await apis.user.favorites.isFavorite(xlsxFileId)).toBe(
                false,
                'Item is still favorite'
            );
            expect(await dataTable.isItemPresent(xlsxFavorites)).toBe(
                false,
                'Item is still present in Favorites list'
            );
        });
        ...
        it('[C286394] - Delete action', async () => {
            ...
            await Utils.pressEscape();
            await page.clickTrashAndWait();
            expect(await dataTable.isItemPresent(pdfFavorites)).toBe(
                true,
                'Item is not present in Trash'
            );
        });
        ...
        it('[C297606] - Upload new version action', async () => {
            ...
            expect(await viewer.getFileTitle()).toContain(docxFile2);
            expect(await apis.user.nodes.getFileVersionType(fileFavId)).toEqual(
                'MAJOR',
                'File has incorrect version type'
            );
            expect(
                await apis.user.nodes.getFileVersionLabel(fileFavId)
            ).toEqual('2.0', 'File has incorrect version label');
        });
