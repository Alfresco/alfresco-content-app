...
describe('Viewer actions', () => {
  ...
  describe('from Personal Files', () => {
    ...
    beforeAll(async () => {
      ...
      docxFileId = (await repo.upload.uploadFileWithRename(docxFile, parentId, docxPersonalFiles)).entry.id;

      filePersonalFilesId = (await repo.upload.uploadFile(docxFile2, parentId)).entry.id;
      await repo.upload.uploadFileWithRename(xlsxFileForMove, parentId, xlsxPersonalFiles);
      await repo.upload.uploadFileWithRename(pdfFileForDelete, parentId, pdfPersonalFiles);

      fileForEditOfflineId = (await repo.upload.uploadFileWithRename(docxFile, parentId, fileForEditOffline)).entry.id;
      fileForCancelEditingId = (await repo.upload.uploadFileWithRename(docxFile, parentId, fileForCancelEditing)).entry.id;
      fileForUploadNewVersionId = (await repo.upload.uploadFileWithRename(docxFile, parentId, fileForUploadNewVersion)).entry.id;
      fileForUploadNewVersionId2 = (await repo.upload.uploadFileWithRename(docxFile, parentId, fileForUploadNewVersion2)).entry.id;
    ...
    it('[C268132] Favorite action', async () => {
      ...
      expect(await repo.favorites.isFavorite(docxFileId)).toBe(true, 'Item is not favorite');
    ...
    it('[C297584] Edit Offline action', async () => {
      ...
      expect(await repo.nodes.isFileLockedWrite(fileForEditOfflineId)).toBe(true, `${fileForEditOffline} is not locked`);
    ...
    it('[C297585] Cancel Editing action', async () => {
      ...
      expect(await repo.nodes.isFileLockedWriteWithRetry(fileForCancelEditingId, false)).toBe(false, `${fileForCancelEditing} is still locked`);
      expect(await viewer.isViewerOpened()).toBe(true, 'Viewer is not open');
    });
    ...
    it('[C297586] Upload new version action', async () => {
      ...
      expect(await repo.nodes.getFileVersionType(filePersonalFilesId)).toEqual('MAJOR', 'File has incorrect version type');
      expect(await repo.nodes.getFileVersionLabel(filePersonalFilesId)).toEqual('2.0', 'File has incorrect version label');
    });
  ...
  describe('from File Libraries', () => {
    ...
    beforeAll(async () => {
      ...
      destinationId = (await repo.nodes.createFolder(destination)).entry.id;
      docxFileId = (await repo.upload.uploadFileWithRename(docxFile, docLibId, docxLibraries)).entry.id;

      fileLibrariesId = (await repo.upload.uploadFile(docxFile2, docLibId)).entry.id;

      await repo.upload.uploadFileWithRename(xlsxFileForMove, docLibId, xlsxLibraries);
      await repo.upload.uploadFileWithRename(pdfFileForDelete, docLibId, pdfLibraries);

      fileForEditOfflineId = (await repo.upload.uploadFileWithRename(docxFile, docLibId, fileForEditOffline)).entry.id;
      fileForCancelEditingId = (await repo.upload.uploadFileWithRename(docxFile, docLibId, fileForCancelEditing)).entry.id;
      fileForUploadNewVersionId = (await repo.upload.uploadFileWithRename(docxFile, docLibId, fileForUploadNewVersion)).entry.id;
    ...
    it('[C286372] Favorite action', async () => {
      ...
      await viewer.closeButton.click();
      await page.clickFavoritesAndWait();
      expect(await repo.favorites.isFavorite(docxFileId)).toBe(true, `${docxLibraries} is not favorite`);
    ...
    it('[C297591] Upload new version action', async () => {
      ...
      expect(await viewer.isViewerOpened()).toBe(true, 'Viewer is not open');
      expect(await viewer.getFileTitle()).toContain(docxFile2);
      expect(await repo.nodes.getFileVersionType(fileLibrariesId)).toEqual('MAJOR', 'File has incorrect version type');
      expect(await repo.nodes.getFileVersionLabel(fileLibrariesId)).toEqual('2.0', 'File has incorrect version label');
    });
  ...
  describe('from Recent Files', () => {
    ...
    beforeAll(async () => {
      ...
      parentId = (await repo.nodes.createFolder(parent)).entry.id;
      destinationId = (await repo.nodes.createFolder(destination)).entry.id;
      docxFileId = (await repo.upload.uploadFileWithRename(docxFile, parentId, docxRecentFiles)).entry.id;

      fileRecentId = (await repo.upload.uploadFile(docxFile2, parentId)).entry.id;
    ...
    it('[C297596] Upload new version action', async () => {
      ...
      expect(await viewer.isViewerOpened()).toBe(true, 'Viewer is not open');
      expect(await viewer.getFileTitle()).toContain(docxFile2);
      expect(await repo.nodes.getFileVersionType(fileRecentId)).toEqual('MAJOR', 'File has incorrect version type');
      expect(await repo.nodes.getFileVersionLabel(fileRecentId)).toEqual('2.0', 'File has incorrect version label');
    });
  ...
  describe('from Shared Files', () => {
    ...
    beforeAll(async () => {
      ...
      xlsxFileId = (await repo.upload.uploadFileWithRename(xlsxFileForMove, parentId, xlsxSharedFiles)).entry.id;
      pdfFileId = (await repo.upload.uploadFileWithRename(pdfFileForDelete, parentId, pdfSharedFiles)).entry.id;
      fileSharedId = (await repo.upload.uploadFile(docxFile2, parentId)).entry.id;
      ...
      await repo.shared.shareFilesByIds([
        docxFileId,
        xlsxFileId,
        pdfFileId,
    ...
    it('[C297603] Upload new version action', async () => {
      ...
      expect(await viewer.isViewerOpened()).toBe(true, 'Viewer is not open');
      expect(await viewer.getFileTitle()).toContain(docxFile2);
      expect(await repo.nodes.getFileVersionType(fileSharedId)).toEqual('MAJOR', 'File has incorrect version type');
      expect(await repo.nodes.getFileVersionLabel(fileSharedId)).toEqual('2.0', 'File has incorrect version label');
    });
  ...
  describe('from Favorites', () => {
    ...
    beforeAll(async () => {
      ...
      xlsxFileId = (await repo.upload.uploadFileWithRename(xlsxFileForMove, parentId, xlsxFavorites)).entry.id;
      pdfFileId = (await repo.upload.uploadFileWithRename(pdfFileForDelete, parentId, pdfFavorites)).entry.id;
      fileFavId = (await repo.upload.uploadFile(docxFile2, parentId)).entry.id;
      ...
      await repo.favorites.addFavoritesByIds('file', [
        docxFileId,
        xlsxFileId,
        pdfFileId,
        fileForEditOfflineId,
    ...
    it('[C286393] Favorite action', async () => {
      ...
      await viewer.closeButton.click();
      await page.clickFavoritesAndWait();
      expect(await repo.favorites.isFavorite(xlsxFileId)).toBe(false, 'Item is still favorite');
    ...
    it('[C297606] Upload new version action', async () => {
      ...
      expect(await viewer.isViewerOpened()).toBe(true, 'Viewer is not open');
      expect(await viewer.getFileTitle()).toContain(docxFile2);
      expect(await repo.nodes.getFileVersionType(fileFavId)).toEqual('MAJOR', 'File has incorrect version type');
      expect(await repo.nodes.getFileVersionLabel(fileFavId)).toEqual('2.0', 'File has incorrect version label');
    });
