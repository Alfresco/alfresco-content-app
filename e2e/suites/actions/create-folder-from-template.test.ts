...
describe('Create folder from template', () => {
    ...
    beforeAll(async () => {
        ...
        await adminApiActions.removeUserAccessOnSpaceTemplate(
            restrictedTemplateFolder
        );
        folderLink = (
            await adminApiActions.createLinkToFolderName(
                folderInRootFolder,
                await adminApiActions.getSpaceTemplatesFolderId()
            )
        ).entry.name;
    ...
    describe('Select Template dialog', () => {
        ...
        it(`Templates don't appear if user doesn't have permissions to see them - [C325148]`, async () => {
            ...
            ).toBe(false, 'restricted template folder is displayed');
        });
        ...
        it('Navigate through the templates list with folder hierarchy - [C325149]', async () => {
            ...
            ).toBe(false, 'template folder is displayed');
            expect(
                await selectTemplateDialog.breadcrumb.getCurrentFolderName()
            ...
            ).toEqual([templateFolder2, 'Space Templates']);
        });
        ...
        it(`Templates list doesn't allow multiple selection - [C325150]`, async () => {
            ...
            ).toEqual([templateFolder1], 'Incorrect selected item');
            ...
            expect(
                await selectTemplateDialog.dataTable.getSelectedRowsNames()
            ).toEqual([templateFolder2], 'Incorrect selected item');
        });

        it('Links to folders are not displayed - [C325153]', async () => {
            expect(
                await selectTemplateDialog.dataTable.isItemPresent(folderLink)
            ).toBe(false, 'Link to folder is displayed');
        });
    ...
    describe('Create from template dialog', () => {
        ...
        it('Create folder from template - dialog UI - [C325142]', async () => {
            expect(await createFromTemplateDialog.getTitle()).toEqual(
                `Create new folder from '${templateFolder1}'`
            );
            expect(await createFromTemplateDialog.isNameFieldDisplayed()).toBe(
                true,
                'Name field not displayed'
            );
            expect(await createFromTemplateDialog.isTitleFieldDisplayed()).toBe(
                true,
            ...
            expect(await createFromTemplateDialog.isCancelButtonEnabled()).toBe(
                true,
                'Cancel button is not enabled'
            );
            expect(await createFromTemplateDialog.isCreateButtonEnabled()).toBe(
                true,
    ...
    describe('On Personal Files', () => {
        ...
        it('Create a folder from a template - with a new Name - [C325157]', async () => {
            ...
            expect(await page.dataTable.isItemPresent(folder1.name)).toBe(
                true,
                'Folder not displayed in list view'
            );
        });
        ...
        it('Create a folder from a template - with a Name, Title and Description - [C325154]', async () => {
            ...
            expect(await page.dataTable.isItemPresent(folder2.name)).toBe(
                true,
                'Folder not displayed in list view'
            );
        ...
        it('Cancel folder creation - [C325155]', async () => {
            ...
            expect(await page.dataTable.isItemPresent('test')).toBe(
                false,
                'Folder should not appear in the list'
            );
        });
    ...
    describe('On File Libraries', () => {
        ...
        it('Create a folder from a template - with Name, Title and Description - [C325161]', async () => {
            ...
            await createFromTemplateDialog.enterDescription(
                folderSite.description
            );
            await createFromTemplateDialog.clickCreate();
            await createFromTemplateDialog.waitForDialogToClose();
            await page.dataTable.waitForHeader();

            expect(await page.dataTable.isItemPresent(folderSite.name)).toBe(
                true,
                'Folder not displayed in list view'
            );
            const desc = await userApi.nodes.getNodeDescription(
