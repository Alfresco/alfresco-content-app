...
describe('Download', () => {
    ...
    beforeAll(async (done) => {
        ...
        fileShared1Id = (
            await apis.user.nodes.createFile(fileShared1, parentId)
        ).entry.id;
        fileShared2Id = (
            await apis.user.nodes.createFile(fileShared2, parentId)
        ).entry.id;
        fileFavoritesId = (
            await apis.user.nodes.createFile(fileFavorites, parentId)
        ).entry.id;
        await apis.user.nodes.createFile(fileSearch, parentId);

        folderPersonalId = (
            await apis.user.nodes.createFolder(folderPersonal, parentId)
        ).entry.id;
        await apis.user.nodes.createFile(
            fileInFolderPersonal,
            folderPersonalId
        );

        folderFavoritesId = (
            await apis.user.nodes.createFolder(folderFavorites, parentId)
        ).entry.id;
        await apis.user.nodes.createFile(
            fileInFolderFavorites,
            folderFavoritesId
        );
    ...
    describe('on Personal Files', () => {
        ...
        it('Download a file - [C213179]', async () => {
            ...
            expect(await Utils.fileExistsOnOS(filePersonal)).toBe(
                true,
        ...
        it('Download a folder - [C216352]', async () => {
            ...
            expect(await Utils.fileExistsOnOS(folderZip)).toBe(
                true,
                'File not found in download location'
            );
            ...
            expect(
                await Utils.fileExistsOnOS(fileInFolderPersonal, folderPersonal)
            );
        });
        ...
        it('Download multiple items - [C216353]', async () => {
            ...
            expect(await Utils.fileExistsOnOS(archiveZip)).toBe(
                true,
                'File not found in download location'
            );
            ...
            expect(
                await Utils.fileExistsOnOS(filePersonal, unzippedPersonal)
            ).toBe(true, `${filePersonal} not found in unzipped folder`);
            expect(
                await Utils.fileExistsOnOS(folderPersonal, unzippedPersonal)
            ).toBe(true, `${folderPersonal} not found in unzipped folder`);
            expect(
                await Utils.fileExistsOnOS(
                    fileInFolderPersonal,
                    unzippedPersonal,
                    folderPersonal
                )
    ...
    describe('on Favorites', () => {
        ...
        it('Download a file - [C280173]', async () => {
            ...
            expect(await Utils.fileExistsOnOS(fileFavorites)).toBe(
                true,
                'File not found in download location'
            );
        });
        ...
        it('Download multiple items - [C280189]', async () => {
            ...
            expect(
                await Utils.fileExistsOnOS(fileFavorites, unzippedFavorites)
            ).toBe(true, `${fileFavorites} not found in unzipped folder`);
            expect(
                await Utils.fileExistsOnOS(folderFavorites, unzippedFavorites)
            ).toBe(true, `${folderFavorites} not found in unzipped folder`);
            expect(
                await Utils.fileExistsOnOS(
                    fileInFolderFavorites,
                    unzippedFavorites,
    ...
    describe('on Shared Files', () => {
        ...
        it('Download a file - [C280170]', async () => {
            ...
            expect(await Utils.fileExistsOnOS(fileShared1)).toBe(
                true,
                'File not found in download location'
            );
        });
        ...
        it('Download multiple items - [C280183]', async () => {
            ...
            expect(
                await Utils.fileExistsOnOS(fileShared1, unzippedShared)
            ).toBe(true, `${fileShared1} not found in unzipped folder`);
            expect(
                await Utils.fileExistsOnOS(fileShared2, unzippedShared)
            ).toBe(true, `${fileShared2} not found in unzipped folder`);
        });
    });
    ...
    describe('on Recent Files', () => {
        ...
        it('Download a file - [C280167]', async () => {
            ...
            expect(await Utils.fileExistsOnOS(fileRecent1)).toBe(
                true,
                'File not found in download location'
            );
        });
        ...
        it('Download multiple items - [C280177]', async () => {
            ...
            expect(
                await Utils.fileExistsOnOS(fileRecent1, unzippedRecent)
            ).toBe(true, `${fileRecent1} not found in unzipped folder`);
            expect(
                await Utils.fileExistsOnOS(fileRecent2, unzippedRecent)
            ).toBe(true, `${fileRecent2} not found in unzipped folder`);
        });
    });
    ...
    describe('on Search Results', () => {
        ...
        it('Download a file - [C279164]', async () => {
            ...
            expect(await Utils.fileExistsOnOS(fileSearch)).toBe(
                true,
                'File not found in download location'
            );
        });
        ...
        it('Download a folder - [C297694]', async () => {
            ...
            expect(
                await Utils.fileExistsOnOS(fileInFolderSearch, folderSearch)
            );
        });
        ...
        it('Download multiple items - [C297695]', async () => {
            ...
            expect(
                await Utils.fileExistsOnOS(folderSearch, unzippedSearch)
            ).toBe(true, `${folderSearch} not found in unzipped folder`);
            expect(
                await Utils.fileExistsOnOS(
                    fileInFolderSearch,
                    unzippedSearch,
