...
describe('Move content', () => {
    ...
    beforeAll(async (done) => {
        ...
        destinationIdPF = (await apis.user.nodes.createFolder(destinationPF))
            .entry.id;

        sourceIdRF = (await apis.user.nodes.createFolder(sourceRF)).entry.id;
        destinationIdRF = (await apis.user.nodes.createFolder(destinationRF))
            .entry.id;

        sourceIdSF = (await apis.user.nodes.createFolder(sourceSF)).entry.id;
        destinationIdSF = (await apis.user.nodes.createFolder(destinationSF))
            .entry.id;

        sourceIdFav = (await apis.user.nodes.createFolder(sourceFav)).entry.id;
        destinationIdFav = (await apis.user.nodes.createFolder(destinationFav))
            .entry.id;
    ...
    describe('from Personal Files', () => {
        ...
        beforeAll(async (done) => {
            ...
            folder1Id = (
                await apis.user.nodes.createFolder(folder1, sourceIdPF)
            ).entry.id;
            ...
            existingId1 = (
                await apis.user.nodes.createFolder(existingFolder, sourceIdPF)
            ).entry.id;
            existingId2 = (
            ...
            folder2Id = (
                await apis.user.nodes.createFolder(folder2, sourceIdPF)
            ).entry.id;
        ...
        it('Move a file - [C217316]', async () => {
            ...
            expect(await dataTable.isItemPresent(file1)).toBe(
                false,
                `${file1} still present in source folder`
            );
            ...
            expect(await dataTable.isItemPresent(file1)).toBe(
                true,
                `${file1} not present in destination folder`
            );
        });
        ...
        it('Move a folder with content - [C217317]', async () => {
            ...
            expect(await dataTable.isItemPresent(folder1)).toBe(
                false,
                `${folder1} still present in source folder`
            );
            ...
            expect(await dataTable.isItemPresent(folder1)).toBe(
                true,
                `${folder1} not present in destination folder`
            );
            expect(await dataTable.isItemPresent(fileInFolder)).toBe(
                false,
                `${fileInFolder} is present in destination folder`
            );

            await dataTable.doubleClickOnRowByName(folder1);
            expect(await dataTable.isItemPresent(fileInFolder)).toBe(
                true,
                `${fileInFolder} is not present in parent folder`
            );
        });
        ...
        it('Move multiple items - [C291958]', async () => {
            ...
            expect(await dataTable.isItemPresent(file2)).toBe(
                false,
                `${file2} still present in source folder`
            );
            expect(await dataTable.isItemPresent(file3)).toBe(
                false,
                `${file3} still present in source folder`
            );
            ...
            expect(await dataTable.isItemPresent(file2)).toBe(
                true,
                `${file2} not present in destination folder`
            );
            expect(await dataTable.isItemPresent(file3)).toBe(
                true,
                `${file3} not present in destination folder`
            );
        });
        ...
        it('Move a file with a name that already exists on the destination - [C217318]', async () => {
            ...
            expect(msg).toContain(
                'Move unsuccessful, a file with the same name already exists'
            );
            expect(msg).not.toContain('Undo');

            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(`${existingFile}.txt`)).toBe(
                true,
                `${existingFile}.txt not present in source folder`
            );
            ...
            expect(await dataTable.isItemPresent(`${existingFile}.txt`)).toBe(
                true,
                `${existingFile}.txt not present in destination folder`
            );
            expect(await dataTable.isItemPresent(`${existingFile}-1.txt`)).toBe(
                false,
                `${existingFile}-1.txt is present in destination folder`
            );
        });
        ...
        it('Move a folder with a name that already exists on the destination - [C217319]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(existingFolder)).toBe(
                false,
                `${existingFolder} still present in source folder`
            );
            ...
            expect(await dataTable.isItemPresent(existingFolder)).toBe(
                true,
                `${existingFolder} not present in destination folder`
            );
            await dataTable.doubleClickOnRowByName(existingFolder);
            expect(await dataTable.isItemPresent(file2InFolder)).toBe(
                true,
                `${file2InFolder} not present in destination folder`
            );
            expect(await dataTable.isItemPresent(file3InFolder)).toBe(
                true,
                `${file3InFolder} not present in destination folder`
            );
        });
        ...
        it('Move items into a library - [C291969]', async () => {
            ...
            await moveDialog.dataTable.doubleClickOnRowByName(
                'documentLibrary'
            );
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(file4)).toBe(
                false,
                `${file4} still present in source folder`
            );
            expect(await dataTable.isItemPresent(folder2)).toBe(
                false,
                `${folder2} still present in source folder`
            );
            ...
            expect(await dataTable.isItemPresent(file4)).toBe(
                true,
                `${file4} not present in destination folder`
            );
            expect(await dataTable.isItemPresent(folder2)).toBe(
                true,
                `${folder2} not present in destination folder`
            );
            await dataTable.doubleClickOnRowByName(folder2);
            expect(await dataTable.isItemPresent(fileInFolder2)).toBe(
                true,
                `${fileInFolder2} not present in parent folder`
            );
        });
    });
    ...
    describe('from Recent Files', () => {
        ...
        it('Move a file - [C280230]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(file1, destinationRF)).toBe(
                true,
                `${file1} from ${destinationRF} not present`
            );
            expect(await dataTable.isItemPresent(file1, sourceRF)).toBe(
                false,
                `${file1} from ${sourceRF} is present`
            );
        ...
        it('Move multiple items - [C280237]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(file2, destinationRF)).toBe(
                true,
                `${file2} from ${destinationRF} not present`
            );
            expect(await dataTable.isItemPresent(file3, destinationRF)).toBe(
                true,
                `${file3} from ${destinationRF} not present`
            );
            expect(await dataTable.isItemPresent(file2, sourceRF)).toBe(
                false,
                `${file2} from ${sourceRF} is present`
            );
            expect(await dataTable.isItemPresent(file3, sourceRF)).toBe(
                false,
                `${file3} from ${sourceRF} is present`
            );
        ...
        it('Move a file with a name that already exists on the destination - [C291970]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(existingFile, sourceRF)).toBe(
                true,
                `${existingFile} from ${sourceRF} not present`
            );
            expect(
                await dataTable.isItemPresent(existingFile, destinationRF)
            ).toBe(true, `${existingFile} from ${destinationRF} not present`);
        ...
        it('Move items into a library - [C291971]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(file4, folderSiteRF)).toBe(
                true,
                `${file4} from ${folderSiteRF} not present`
            );
            expect(await dataTable.isItemPresent(file4, sourceRF)).toBe(
                false,
                `${file4} from ${sourceRF} is present`
            );
    ...
    describe('from Shared Files', () => {
        ...
        beforeAll(async (done) => {
            file1Id = (await apis.user.nodes.createFile(file1, sourceIdSF))
                .entry.id;
            await apis.user.shared.shareFileById(file1Id);

            file2Id = (await apis.user.nodes.createFile(file2, sourceIdSF))
                .entry.id;
            file3Id = (await apis.user.nodes.createFile(file3, sourceIdSF))
                .entry.id;
            ...
            file4Id = (await apis.user.nodes.createFile(file4, sourceIdSF))
                .entry.id;
        ...
        it('Move a file - [C280243]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(file1, destinationSF)).toBe(
                true,
                `${file1} from ${destinationSF} not present`
            );
            expect(await dataTable.isItemPresent(file1, sourceSF)).toBe(
                false,
                `${file1} from ${sourceSF} is present`
            );
        ...
        it('Move multiple items - [C280250]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(file2, destinationSF)).toBe(
                true,
                `${file2} from ${destinationSF} not present`
            );
            expect(await dataTable.isItemPresent(file3, destinationSF)).toBe(
                true,
                `${file3} from ${destinationSF} not present`
            );
            expect(await dataTable.isItemPresent(file2, sourceSF)).toBe(
                false,
                `${file2} from ${sourceSF} is present`
            );
            expect(await dataTable.isItemPresent(file3, sourceSF)).toBe(
                false,
                `${file3} from ${sourceSF} is present`
            );
        ...
        it('Move a file with a name that already exists on the destination - [C291977]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(existingFile, sourceSF)).toBe(
                true,
                `${existingFile} from ${sourceSF} not present`
            );
            expect(
                await dataTable.isItemPresent(existingFile, destinationSF)
            ).toBe(false, `${existingFile} from ${destinationSF} is present`);
            ...
            expect(await dataTable.isItemPresent(`${existingFile}-1.txt`)).toBe(
                false,
                `${existingFile}-1.txt not present in destination folder`
            );
        });
        ...
        it('Move items into a library - [C291978]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(file4, folderSiteSF)).toBe(
                true,
                `${file4} from ${folderSiteSF} not present`
            );
            expect(await dataTable.isItemPresent(file4, sourceSF)).toBe(
                false,
                `${file4} from ${sourceSF} is present`
            );
    ...
    describe('from Favorites', () => {
        ...
        beforeAll(async (done) => {
            file1Id = (await apis.user.nodes.createFile(file1, sourceIdFav))
                .entry.id;
            await apis.user.favorites.addFavoriteById('file', file1Id);

            folder1Id = (
                await apis.user.nodes.createFolder(folder1, sourceIdFav)
            ).entry.id;
            await apis.user.nodes.createFile(fileInFolder, folder1Id);
            await apis.user.favorites.addFavoriteById('folder', folder1Id);

            file2Id = (await apis.user.nodes.createFile(file2, sourceIdFav))
                .entry.id;
            file3Id = (await apis.user.nodes.createFile(file3, sourceIdFav))
                .entry.id;
            ...
            existingId1 = (
                await apis.user.nodes.createFolder(existingFolder, sourceIdFav)
            ).entry.id;
            existingId2 = (
                await apis.user.nodes.createFolder(
                    existingFolder,
            ...
            file4Id = (await apis.user.nodes.createFile(file4, sourceIdFav))
                .entry.id;
            folder2Id = (
                await apis.user.nodes.createFolder(folder2, sourceIdFav)
            ).entry.id;
            await apis.user.nodes.createFile(fileInFolder2, folder2Id);
        ...
        it('Move a file - [C280256]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(file1, destinationFav)).toBe(
                true,
                `${file1} from ${destinationFav} not present`
            );
            expect(await dataTable.isItemPresent(file1, sourceFav)).toBe(
                false,
                `${file1} from ${sourceFav} is present`
            );
        ...
        it('Move a folder with content - [C280257]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(folder1, sourceFav)).toBe(
                false,
                `${folder1} from ${sourceFav} is present`
            );
            expect(await dataTable.isItemPresent(folder1, destinationFav)).toBe(
                true,
                `${folder1} from ${destinationFav} not present`
            );
            ...
            expect(await dataTable.isItemPresent(fileInFolder)).toBe(
                false,
                `${fileInFolder} is present in destination`
            );
        ...
        it('Move multiple items - [C280258]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(file2, destinationFav)).toBe(
                true,
                `${file2} from ${destinationFav} not present`
            );
            expect(await dataTable.isItemPresent(file3, destinationFav)).toBe(
                true,
                `${file3} from ${destinationFav} not present`
            );
            expect(await dataTable.isItemPresent(file2, sourceFav)).toBe(
                false,
                `${file2} from ${sourceFav} is present`
            );
            expect(await dataTable.isItemPresent(file3, sourceFav)).toBe(
                false,
                `${file3} from ${sourceFav} is present`
            );
        ...
        it('Move a file with a name that already exists on the destination - [C280263]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(existingFile, sourceFav)).toBe(
                true,
                `${existingFile} from ${sourceFav} not present`
            );
            expect(
                await dataTable.isItemPresent(existingFile, destinationFav)
            ).toBe(false, `${existingFile} from ${destinationFav} is present`);
        ...
        it('Move a folder with a name that already exists on the destination - [C280259]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(
                await dataTable.isItemPresent(existingFolder, sourceFav)
            ).toBe(false, `${existingFolder} from ${sourceFav} is present`);
        ...
        it('Move items into a library - [C291979]', async () => {
            ...
            await moveDialog.waitForDialogToClose();
            expect(await dataTable.isItemPresent(file4, folderSiteFav)).toBe(
                true,
                `${file4} from ${folderSiteFav} not present`
            );
            expect(await dataTable.isItemPresent(file4, sourceFav)).toBe(
                false,
                `${file4} from ${sourceFav} is present`
            );
            expect(await dataTable.isItemPresent(folder2, folderSiteFav)).toBe(
                true,
                `${folder2} from ${folderSiteFav} not present`
            );
            expect(await dataTable.isItemPresent(folder2, sourceFav)).toBe(
                false,
                `${folder2} from ${sourceFav} is present`
            );
