...
describe('Copy content', () => {
    ...
    beforeAll(async (done) => {
        ...
        destinationIdFav = (await apis.user.nodes.createFolder(destinationFav))
            .entry.id;
        destinationIdSearch = (
            await apis.user.nodes.createFolder(destinationSearch)
        ).entry.id;

        existingFileToCopyId = (
            await apis.user.nodes.createFile(existingFile, sourceId)
        ).entry.id;
        ...
        existingFolderToCopyId = (
            await apis.user.nodes.createFolder(existingFolder, sourceId)
        ).entry.id;

        existingIdPF = (
            await apis.user.nodes.createFolder(existingFolder, destinationIdPF)
        ).entry.id;
        ...
        existingIdFav = (
            await apis.user.nodes.createFolder(existingFolder, destinationIdFav)
        ).entry.id;
        existingIdSearch = (
        ...
        folder1Id = (await apis.user.nodes.createFolder(folder1, sourceId))
            .entry.id;
        fileInFolderId = (
        ...
        folderExistingId = (
            await apis.user.nodes.createFolder(folderExisting, sourceId)
        ).entry.id;
        await apis.user.favorites.addFavoriteById('folder', folderExistingId);
        await apis.user.nodes.createFile(
            file1InFolderExisting,
            folderExistingId
        );

        folderExistingPFId = (
            await apis.user.nodes.createFolder(folderExisting, destinationIdPF)
        ).entry.id;
        await apis.user.nodes.createFile(
            file2InFolderExisting,
            folderExistingPFId
        );

        folderExistingFavId = (
            await apis.user.nodes.createFolder(folderExisting, destinationIdFav)
        ).entry.id;
        await apis.user.nodes.createFile(
            file2InFolderExisting,
            folderExistingFavId
        ...
        folder2Id = (await apis.user.nodes.createFolder(folder2, sourceId))
            .entry.id;
        await apis.user.nodes.createFile(fileInFolder2, folder2Id);
        await apis.user.favorites.addFavoriteById('folder', folder2Id);

        fileLocked1Id = (
            await apis.user.nodes.createFile(fileLocked1, sourceId)
        ).entry.id;
        await apis.user.nodes.lockFile(fileLocked1Id);

        folderWithLockedFilesId = (
            await apis.user.nodes.createFolder(folderWithLockedFiles, sourceId)
        ).entry.id;
        fileLockedInFolderId = (
            await apis.user.nodes.createFile(
                fileLockedInFolder,
                folderWithLockedFilesId
            )
    ...
    describe('from Recent Files', () => {
        ...
        it('Copy a file - [C280194]', async () =>
            copyFile(file1, source, destinationRF));

        it('Copy multiple items - [C280201]', async () =>
            copyMultipleItems([file2, file3], source, destinationRF));

        it('Copy a file with a name that already exists on the destination - [C280196]', async () =>
            copyFileWithNameThatAlreadyExists(
                existingFile,
                source,
                destinationRF
            ));
        ...
        it('Copy items into a library - [C291899]', async () =>
            copyItemsIntoLibrary([file1, file2], source, folderSiteRF));

        it('Copy locked file - [C280198]', async () =>
            copyLockedFile(fileLocked1, source, destinationRF, () => {
                locationId = sourceId;
                destinationId = destinationIdRF;
            }));

        it('Undo copy of files - [C280202]', async () =>
            undoCopyFile(file4, source, destinationRF));
    });
    ...
    describe('from Personal Files', () => {
        ...
        it('Copy a file - [C217135]', async () =>
            copyFile(file1, '', destinationPF));

        it('Copy a folder with content - [C291888]', async () =>
            copyFolderWithContent(folder1, '', destinationPF));

        it('Copy multiple items - [C291889]', async () =>
            copyMultipleItems([file2, file3], '', destinationPF));
        ...
        it('Copy a folder with a name that already exists on the destination - [C217138]', async () =>
            copyFolderWithNameThatAlreadyExists(
                existingFolder,
                '',
                destinationPF
            ));
        ...
        it('Copy items into a library - [C280282]', async () =>
            copyItemsIntoLibrary([file1, folder1], '', folderSitePF));

        it('Copy locked file - [C217139]', async () =>
            copyLockedFile(fileLocked1, '', destinationPF, () => {
                locationId = sourceId;
                destinationId = destinationIdPF;
            }));

        it('Copy folder that contains locked file - [C217140]', async () =>
            copyFolderThatContainsLockedFile(
                folderWithLockedFiles,
                '',
                destinationPF,
                () => {
        ...
        it('Undo copy of files - [C217171]', async () =>
            undoCopyFile(file4, '', destinationPF));

        it('Undo copy of folders - [C217172]', async () =>
            undoCopyFolder(folder2, '', destinationPF));

        it('Undo copy of a file when a file with same name already exists on the destination - [C217173]', async () =>
            undoCopyFileWithExistingName(
                fileInFolder,
                '',
                folder2,
                async () => {
        ...
        it('Undo copy of a folder when a folder with same name already exists on the destination - [C217174]', async () =>
    ...
    describe('from Shared Files', () => {
        ...
        it('Copy a file - [C280206]', async () =>
            copyFile(file1, source, destinationSF));

        it('Copy multiple items - [C280213]', async () =>
            copyMultipleItems([file2, file3], source, destinationSF));
        ...
        it('Copy items into a library - [C291900]', async () =>
            copyItemsIntoLibrary([file1, file2], source, folderSiteSF));

        it('Copy locked file - [C280210]', async () =>
            copyLockedFile(fileLocked1, source, destinationSF, () => {
                locationId = sourceId;
                destinationId = destinationIdSF;
            }));

        it('Undo copy of files - [C280214]', async () =>
            undoCopyFile(file4, source, destinationSF));
    });
    ...
    describe('from Favorites', () => {
        ...
        it('Copy a file - [C280218]', async () =>
            copyFile(file1, source, destinationFav));

        it('Copy a folder with content - [C280219]', async () =>
            copyFolderWithContent(folder1, source, destinationFav));

        it('Copy multiple items - [C280225]', async () =>
            copyMultipleItems([file2, file3], source, destinationFav));
        ...
        it('Copy items into a library - [C291901]', async () =>
            copyItemsIntoLibrary([file1, folder1], source, folderSiteFav));

        it('Copy locked file - [C280222]', async () =>
            copyLockedFile(fileLocked1, source, destinationFav, () => {
                locationId = sourceId;
                destinationId = destinationIdFav;
            }));

        it('Copy folder that contains locked file - [C280223]', async () =>
            copyFolderThatContainsLockedFile(
                folderWithLockedFiles,
                source,
                destinationFav,
        ...
        it('Undo copy of files - [C280226]', async () =>
            undoCopyFile(file4, source, destinationFav));

        it('Undo copy of folders - [C280227]', async () =>
            undoCopyFolder(folder2, source, destinationFav));

        it('Undo copy of a file when a file with same name already exists on the destination - [C280228]', async () =>
            undoCopyFileWithExistingName(fileInFolder, folder1, folder2));

        it('Undo copy of a folder when a folder with same name already exists on the destination - [C280229]', async () =>
            undoCopyFolderWithExistingName(
                folderExisting,
                source,
                destinationFav
            ));
    ...
    async function copyFile(
        ...
        expect(await dataTable.isItemPresent(fileName)).toBe(
            true,
            `${fileName} not present in source folder`
        );
        await page.clickPersonalFilesAndWait();
        await dataTable.doubleClickOnRowByName(destination);
        expect(await dataTable.isItemPresent(fileName)).toBe(
            true,
            `${fileName} not present in ${destination} folder`
        );
    }

    async function copyFolderWithContent(
        folderName: string,
        location: string = '',
        destination: string,
        doBefore = null
        ...
        await copyDialog.waitForDialogToClose();
        expect(await dataTable.isItemPresent(folderName)).toBe(
            true,
            `${folderName} not present in source folder`
        );
        await page.clickPersonalFilesAndWait();
        await dataTable.doubleClickOnRowByName(destination);
        expect(await dataTable.isItemPresent(folderName)).toBe(
            true,
            `${folderName} not present in ${destination} folder`
        );
        ...
        expect(await dataTable.isItemPresent(fileInFolder)).toBe(
            true,
            `${fileInFolder} is not present in ${folderName} folder in ${destination}`
        );
    }
    ...
    async function copyMultipleItems(
        ...
        await copyDialog.waitForDialogToClose();
        expect(await dataTable.isItemPresent(items[0])).toBe(
            true,
            `${items[0]} not present in source folder`
        );
        expect(await dataTable.isItemPresent(items[1])).toBe(
            true,
            `${items[1]} not present in source folder`
        );
        await page.clickPersonalFilesAndWait();
        await dataTable.doubleClickOnRowByName(destination);
        expect(await dataTable.isItemPresent(items[0])).toBe(
            true,
            `${items[0]} not present in ${destination} folder`
        );
        expect(await dataTable.isItemPresent(items[1])).toBe(
            true,
            `${items[1]} not present in ${destination} folder`
        );
    }

    async function copyFileWithNameThatAlreadyExists(
        fileName: string,
        location: string = '',
        destination: string,
        doBefore = null
        ...
        expect(await dataTable.isItemPresent(fileName)).toBe(
            true,
            `${fileName}.txt not present in source folder`
        );
        await page.clickPersonalFilesAndWait();
        await dataTable.doubleClickOnRowByName(destination);
        expect(await dataTable.isItemPresent(fileName)).toBe(
            true,
            `${fileName}.txt not present in ${destination} folder`
        );
    }

    async function copyFolderWithNameThatAlreadyExists(
        folderName: string,
        location: string = '',
        destination: string,
        doBefore = null
        ...
        expect(await dataTable.isItemPresent(file2InFolder)).toBe(
            true,
            `${file2InFolder} not present in ${destination} folder in ${folderName}`
        );
        expect(await dataTable.isItemPresent(file3InFolder)).toBe(
            true,
            `${file3InFolder} not present in ${destination} folder in ${folderName}`
        );
    }
    ...
    async function copyItemsIntoLibrary(
        ...
        expect(msg).toContain(
            `Copied ${noOfItems} ${noOfItems === 1 ? 'item' : 'items'}`
        );
        expect(msg).toContain('Undo');

        await copyDialog.waitForDialogToClose();
        for (const item of items) {
            expect(await dataTable.isItemPresent(item)).toBe(
                true,
                `${item} not present in source folder`
            );
        }
        ...
        for (const item of items) {
            expect(await dataTable.isItemPresent(item)).toBe(
                true,
                `${item} not present in ${destination} folder`
            );
        }
    }
    ...
    async function copyLockedFile(
        ...
        expect(
            await apis.user.nodes.isFileLockedByName(fileName, locationId)
        ).toBe(true, `${fileName} not locked in ${location}`);
        await page.clickPersonalFilesAndWait();
        await dataTable.doubleClickOnRowByName(destination);
        expect(await dataTable.isItemPresent(fileName)).toBe(
            true,
        ...
        expect(
            await apis.user.nodes.isFileLockedByName(fileName, destinationId)
        ).toBe(false, `${fileName} is locked in ${destination}`);
    }
    ...
    async function copyFolderThatContainsLockedFile(
        ...
        expect(await dataTable.isItemPresent(fileLockedInFolder)).toBe(
            false,
            `${fileLockedInFolder} is present in ${destination}`
        );
        expect(
            await apis.user.nodes.isFileLockedByName(
                fileLockedInFolder,
                locationId
            )
        ).toBe(true, `${fileLockedInFolder} not locked in ${location}`);
        ...
        expect(
            await apis.user.nodes.isFileLockedByName(
                fileLockedInFolder,
                await apis.user.nodes.getNodeIdFromParent(
                    folderWithLockedFiles,
                    destinationId
                )
            )
    ...
    async function undoCopyFile(
        ...
        expect(await dataTable.isItemPresent(fileName)).toBe(
            false,
            `${fileName} present in ${destination} folder`
        );
    ...
    async function undoCopyFolder(
        ...
        expect(await dataTable.isItemPresent(folderName)).toBe(
            false,
            `${folderName} present in ${destination} folder`
        );
    ...
    async function undoCopyFileWithExistingName(
        fileName: string,
        location: string = '',
        destination: string,
        doBefore = null
        ...
        expect(await dataTable.isItemPresent(fileInFolder2)).toBe(
            true,
            `${fileInFolder2} not present in ${destination} folder`
        );
        expect(await dataTable.isItemPresent(`${fileInFolder2}-1`)).toBe(
            false,
            `${fileInFolder2}-1 is present in ${destination} folder`
        );
    ...
    async function undoCopyFolderWithExistingName(
        folderName: string,
        location: string = '',
        destination: string,
        doBefore = null
