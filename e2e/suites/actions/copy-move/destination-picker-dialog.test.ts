...
describe('Destination picker dialog : ', () => {
    ...
    beforeAll(async () => {
        ...
        destinationId = (await userApi.nodes.createFolder(destination)).entry
            .id;
        ...
        const folder2Id = (
            await userApi.nodes.createFolder(
                folder2InDestination,
                destinationId
            )
        ).entry.id;
        folderLink = (
            await userApi.nodes.createFolderLink(folder2Id, destinationId)
        ).entry.name;
        searchFolderId = (
            await userApi.nodes.createFolder(searchFolder, destinationId)
        ).entry.id;
        searchSubFolder1Id = (
            await userApi.nodes.createFolder(searchSubFolder1, searchFolderId)
        ).entry.id;
        ...
        searchFolderSiteId = (
            await userApi.nodes.createFolder(searchFolder, docLibId)
        ).entry.id;
        searchSubFolder1SiteId = (
            await userApi.nodes.createFolder(
                searchSubFolder1,
                searchFolderSiteId
            )
        ...
        fileIdContributor = (await contributorApi.nodes.createFile(file)).entry
            .id;
        fileIdCollaborator = (await collaboratorApi.nodes.createFile(file))
            .entry.id;

        adminFolderId = (await adminApiActions.nodes.createFolder(adminFolder))
            .entry.id;
    ...
    describe('general', () => {
        ...
        it('Dialog UI - [C263875]', async () => {
            expect(await contentNodeSelector.getTitle()).toEqual(
                `Copy '${file}' to...`
            );
            expect(await contentNodeSelector.isSearchInputPresent()).toBe(
                true,
                'Search input is not displayed'
            );
            expect(
                await contentNodeSelector.isSelectLocationDropdownDisplayed()
            ).toBe(true, 'Select Location dropdown not displayed');
            expect(
                await contentNodeSelector.breadcrumb.getCurrentFolderName()
            ).toEqual('Personal Files');
            expect(
                await contentNodeSelector.dataTable.isItemPresent(destination)
            ).toBe(true, 'Personal Files content not displayed');
            expect(await contentNodeSelector.isCopyButtonEnabled()).toBe(
                true,
                'Copy button is not disabled'
            );
            expect(await contentNodeSelector.isCancelButtonEnabled()).toBe(
                true,
        ...
        it('Files are not displayed - [C263880]', async () => {
            ...
            expect(
                await contentNodeSelector.dataTable.isItemPresent(destination)
            ).toBe(true, 'destination folder not displayed');
            await contentNodeSelector.dataTable.doubleClickOnRowByName(
                destination
            );
            expect(
                await contentNodeSelector.dataTable.isItemPresent(
                    folderInDestination
                )
            ).toBe(true, 'folder is not displayed');
            expect(
                await contentNodeSelector.dataTable.isItemPresent(
                    fileInDestination
                )
            ).toBe(false, 'file is displayed');
        });
        ...
        it('Folder links are not displayed - [C263881]', async () => {
            ...
            ).toBe(true, `${folderInDestination} is not displayed`);
            expect(
                await contentNodeSelector.dataTable.isItemPresent(
                    folder2InDestination
                )
            ).toBe(true, `${folder2InDestination} is not displayed`);
            expect(
                await contentNodeSelector.dataTable.isItemPresent(folderLink)
        ...
        it('User can see his Libraries - [C263885]', async () => {
            ...
            expect(
                await contentNodeSelector.dataTable.isItemPresent(site)
            ).toBe(true, 'user site is not displayed');
        });

        it('Search - No results displayed - [C263889]', async () => {
            await contentNodeSelector.searchFor('nonexistent-folder');
            expect(await contentNodeSelector.dataTable.isEmpty()).toBe(
                true,
                'datatable not empty'
            );
            expect(
                await contentNodeSelector.dataTable.getEmptyListText()
        ...
        it('Search - results found - [C263888]', async () => {
            ...
            ).toBe(true, 'folder from Personal Files not displayed');
            expect(
                await contentNodeSelector.dataTable.isItemPresent(
                    searchFolder,
                    site
                )
            ).toBe(true, 'folder from site not displayed');
        });
    });
    ...
    describe('breadcrumb', () => {
        ...
        it('File Libraries breadcrumb - main node - [C263891]', async () => {
            ...
            expect(
                await contentNodeSelector.breadcrumb.getCurrentFolderName()
            ).toEqual('File Libraries');
        });

        it('Search results breadcrumb - [C263899]', async () => {
            await contentNodeSelector.searchFor(searchFolder);
            expect(await contentNodeSelector.getToolbarTitle()).toEqual(
                'Search results'
            );
        });
        ...
        it('Personal Files breadcrumb - folder structure - [C263897]', async () => {
            ...
            expect(
                await contentNodeSelector.breadcrumb.getCurrentFolderName()
            ).toEqual(searchSubFolder1);
            await contentNodeSelector.dataTable.doubleClickOnRowByName(
                searchSubFolder2
            );
            expect(
                await contentNodeSelector.breadcrumb.getCurrentFolderName()
            ).toEqual(searchSubFolder2);
            await contentNodeSelector.breadcrumb.openPath();
            expect(
                await contentNodeSelector.breadcrumb.getPathItems()
            ).toEqual([
                searchSubFolder1,
                searchFolder,
                destination,
        ...
        it('File Libraries breadcrumb - folder structure - [C263898]', async () => {
            ...
            expect(
                await contentNodeSelector.breadcrumb.getPathItems()
            ).toEqual([searchSubFolder1, searchFolder, site, 'File Libraries']);
        });
        ...
        it('Select a node from the breadcrumb path - [C263895]', async () => {
            ...
            expect(
                await contentNodeSelector.dataTable.isItemPresent(searchFolder)
            ).toBe(true, 'folder not displayed');
        });
    });
