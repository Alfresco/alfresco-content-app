...
import {
    RepoClient,
    NodeContentTree,
} from '../../utilities/repo-client/repo-client';
...
describe('Create file from template', () => {
    ...
    it('Select template - dialog UI - when no templates exist in the repo - [C325049]', async () => {
        ...
        expect(await selectTemplateDialog.getTitle()).toEqual(
            'Select a document template'
        );
        expect(await selectTemplateDialog.dataTable.isEmpty()).toBe(
            true,
            'Datatable is not empty'
        );
        expect(await selectTemplateDialog.dataTable.getEmptyListText()).toEqual(
            'No results found'
        );
        expect(
            await selectTemplateDialog.breadcrumb.getCurrentFolderName()
        ).toEqual('Node Templates');
        expect(await selectTemplateDialog.isNextButtonEnabled()).toBe(
            false,
            'Next button is not disabled'
        );
        expect(await selectTemplateDialog.isCancelButtonEnabled()).toBe(
            true,
    ...
    describe('with existing templates', () => {
        ...
        beforeAll(async () => {
            ...
            await adminApiActions.removeUserAccessOnNodeTemplate(
        ...
        describe('Select Template dialog', () => {
            ...
            it('Select template - dialog UI - with existing templates - [C325043]', async () => {
                ...
                expect(
                    await selectTemplateDialog.dataTable.isItemPresent(
                        templatesFolder1
                    )
                ).toBe(true, 'template folder not displayed');
                expect(
                    await selectTemplateDialog.dataTable.isItemPresent(
                        templatesFolder2
                    )
                ...
                ).toBe(true, 'template not displayed');
                expect(
                    await selectTemplateDialog.dataTable.isItemPresent(
                        template2InRootFolder
                    )
            ...
            it(`Templates don't appear if user doesn't have permissions to see them - [C325044]`, async () => {
                ...
                ).toBe(false, 'restricted templates folder is displayed');
            });
            ...
            it('Navigate through the templates list with folder hierarchy - [C325045]', async () => {
                ...
                await selectTemplateDialog.dataTable.doubleClickOnRowByName(
                    templatesFolder2
                );
                ...
                ).toBe(true, 'template sub-folder not displayed');
                expect(
                    await selectTemplateDialog.dataTable.isItemPresent(
                        template1InFolder2
                    )
                ...
                ).toBe(false, 'template is displayed');
                expect(
                    await selectTemplateDialog.dataTable.isItemPresent(
                        template2InRootFolder
                    )
                ...
                expect(
                    await selectTemplateDialog.breadcrumb.getPathItems()
                ).toEqual([templatesFolder2, 'Node Templates']);
            });

            it(`Templates list doesn't allow multiple selection - [C325047]`, async () => {
                expect(
                    await selectTemplateDialog.dataTable.getSelectedRowsCount()
                ).toEqual(0, 'Incorrect number of selected rows');

                await selectTemplateDialog.dataTable.selectItem(
                    template1InRootFolder
                );
                expect(
                    await selectTemplateDialog.dataTable.getSelectedRowsCount()
                ).toEqual(1, 'Incorrect number of selected rows');
                expect(
                    await selectTemplateDialog.dataTable.getSelectedRowsNames()
                ).toEqual([template1InRootFolder], 'Incorrect selected item');
                ...
                expect(
                    await selectTemplateDialog.dataTable.getSelectedRowsNames()
                ).toEqual([template2InRootFolder], 'Incorrect selected item');
            });

            it('Links to files are not displayed - [C325050]', async () => {
                expect(
                    await selectTemplateDialog.dataTable.isItemPresent(link)
                ).toBe(false, 'Link to file is displayed');
            });
            ...
            it('Cancel the Select template dialog - [C325048]', async () => {
                ...
                expect(await selectTemplateDialog.isDialogOpen()).toBe(
                    false,
                    'Select Template dialog is open'
                );
            });
        ...
        describe('Create from template dialog', () => {
            ...
            it('Create file from template - dialog UI - [C325020]', async () => {
                ...
                expect(
                    await createFromTemplateDialog.isNameFieldDisplayed()
                ).toBe(true, 'Name field not displayed');
                expect(
                    await createFromTemplateDialog.isTitleFieldDisplayed()
                ).toBe(true, 'Title field not displayed');
                expect(
                    await createFromTemplateDialog.isDescriptionFieldDisplayed()
                ).toBe(true, 'Description field not displayed');
                expect(
                    await createFromTemplateDialog.isCancelButtonEnabled()
                ).toBe(true, 'Cancel button is not enabled');
                expect(
                    await createFromTemplateDialog.isCreateButtonEnabled()
                ).toBe(true, 'Create button is not enabled');
            });

            it('File name is required - [C325031]', async () => {
                expect(await createFromTemplateDialog.getName()).toEqual(
                    template1InRootFolder
                );
                await createFromTemplateDialog.deleteNameWithBackspace();

                expect(
                    await createFromTemplateDialog.getValidationMessage()
                ).toEqual('Name is required');
                expect(
                    await createFromTemplateDialog.isCreateButtonEnabled()
                ).toBe(false, 'Create button is not disabled');
            });
            ...
            it('Special characters in file name - [C325032]', async () => {
                ...
                for (const name of namesWithSpecialChars) {
                    ...
                    expect(
                        await createFromTemplateDialog.getValidationMessage()
                    ).toContain(`Name can't contain these characters`);
                }
            });
            ...
            it('File name ending with a dot - [C325033]', async () => {
                ...
                expect(
                    await createFromTemplateDialog.getValidationMessage()
                ).toMatch(`Name can't end with a period .`);
            });
            ...
            it('File name containing only spaces - [C325034]', async () => {
                ...
                expect(
                    await createFromTemplateDialog.getValidationMessage()
                ).toMatch(`Name can't contain only spaces`);
            });
            ...
            it('Title too long - [C290146]', async () => {
                ...
                expect(
                    await createFromTemplateDialog.getValidationMessage()
                ).toMatch(`Use 256 characters or less for title`);
            });

            it('Description too long - [C290142]', async () => {
                await createFromTemplateDialog.enterDescription(
                    Utils.string513
                );
                await Utils.pressTab();
                ...
                expect(
                    await createFromTemplateDialog.getValidationMessage()
                ).toMatch(`Use 512 characters or less for description`);
            });
        });
        ...
        describe('On Personal Files', () => {
            ...
            it('Create a file from a template - with a new Name - [C325030]', async () => {
                ...
                expect(await page.dataTable.isItemPresent(file1.name)).toBe(
                    true,
                    'File not displayed in list view'
                );
            });
            ...
            it('Create a file from a template - with a Name, Title and Description - [C325026]', async () => {
                ...
                expect(await page.dataTable.isItemPresent(file2.name)).toBe(
                    true,
                    'File not displayed in list view'
                );
                const desc = await userApi.nodes.getNodeDescription(
                    file2.name,
                    parentId
                );
            ...
            it('Create a file with a duplicate name - [C325028]', async () => {
                ...
                expect(await createFromTemplateDialog.isDialogOpen()).toBe(
                    true,
            ...
            it('Cancel file creation - [C325027]', async () => {
                ...
                expect(await createFromTemplateDialog.isDialogOpen()).not.toBe(
                    true,
                    'dialog is not closed'
                );
                expect(await page.dataTable.isItemPresent('test')).toBe(
                    false,
            ...
            it('Trim spaces from file Name - [C325042]', async () => {
                ...
                expect(
                    await page.dataTable.isItemPresent(nameWithSpaces.trim())
                ).toBe(true, 'File not displayed in list view');
            });
        });
        ...
        describe('On File Libraries', () => {
            ...
            it('Create a file from a template - with Name, Title and Description - [C325023]', async () => {
                ...
                await createFromTemplateDialog.enterDescription(
                    fileSite.description
                );
                await createFromTemplateDialog.clickCreate();
                await createFromTemplateDialog.waitForDialogToClose();
                await page.dataTable.waitForHeader();

                expect(await page.dataTable.isItemPresent(fileSite.name)).toBe(
                    true,
                    'File not displayed in list view'
                );
                const desc = await userApi.nodes.getNodeDescription(
