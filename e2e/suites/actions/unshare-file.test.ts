...
describe('Unshare a file', () => {
    ...
    describe('from Personal Files', () => {
        ...
        beforeAll(async (done) => {
            file1Id = (await apis.user.nodes.createFile(file1, parentId)).entry
                .id;
            file2Id = (await apis.user.nodes.createFile(file2, parentId)).entry
                .id;
            file3Id = (await apis.user.nodes.createFile(file3, parentId)).entry
                .id;
            file4Id = (await apis.user.nodes.createFile(file4, parentId)).entry
                .id;
        ...
        it('Unshare dialog UI - [C286339]', async () => {
            ...
            expect(await confirmDialog.isRemoveEnabled()).toBe(
                true,
                'REMOVE button is not enabled'
            );
        ...
        it('Unshare a file - [C286340]', async () => {
            ...
            expect(await apis.user.nodes.isFileShared(file2Id)).toBe(
                false,
                `${file2} is shared`
            );
        ...
        it('Unshare a file from the context menu - [C286359]', async () => {
            ...
            expect(await apis.user.nodes.isFileShared(file4Id)).toBe(
                false,
                `${file4} is shared`
            );
    ...
    describe('from File Libraries', () => {
        ...
        beforeAll(async (done) => {
            ...
            file1Id = (await apis.user.nodes.createFile(file1, parentInSiteId))
                .entry.id;
            file2Id = (await apis.user.nodes.createFile(file2, parentInSiteId))
                .entry.id;
            file3Id = (await apis.user.nodes.createFile(file3, parentInSiteId))
                .entry.id;
            file4Id = (await apis.user.nodes.createFile(file4, parentInSiteId))
                .entry.id;
            await apis.user.shared.shareFileById(file1Id);
            await apis.user.shared.shareFileById(file2Id);
            await apis.user.shared.shareFileById(file3Id);
    ...
    describe('as Consumer', () => {
        ...
        beforeAll(async (done) => {
            ...
            file1FileLibId = (
                await apis.admin.nodes.createFile(file1FileLib, docLibId)
            ).entry.id;
            file2FileLibId = (
                await apis.admin.nodes.createFile(file2FileLib, docLibId)
            ).entry.id;
            file1SharedId = (
                await apis.admin.nodes.createFile(file1Shared, docLibId)
            ).entry.id;
            file2SharedId = (
                await apis.admin.nodes.createFile(file2Shared, docLibId)
            ).entry.id;
            file1FavId = (await apis.admin.nodes.createFile(file1Fav, docLibId))
                .entry.id;
            file2FavId = (await apis.admin.nodes.createFile(file2Fav, docLibId))
                .entry.id;
        ...
        it('on File Libraries - file shared by other user - [C286682]', async () => {
            ...
            expect(await shareDialog.isShareToggleDisabled()).toBe(
                false,
                'Share toggle disabled for consumer'
            );
        ...
        it('on File Libraries - file shared by the user - [C286701]', async () => {
            ...
            expect(await apis.user.nodes.isFileShared(file2FileLibId)).toBe(
                false,
                `${file2FileLib} is shared`
            );
        });
        ...
        it('on Shared Files - file shared by the user - [C286702]', async () => {
            ...
            expect(await apis.user.nodes.isFileShared(file2SharedId)).toBe(
                false,
                `${file2Shared} is shared`
            );
        });
        ...
        it('on Favorites - file shared by the user - [C286703]', async () => {
            ...
            expect(await apis.user.nodes.isFileShared(file2FavId)).toBe(
                false,
                `${file2Fav} is shared`
            );
        });
