...
describe('Unshare a file from Search Results', () => {
    ...
    beforeAll(async (done) => {
        ...
        fileSite1Id = (await apis.admin.nodes.createFile(fileSite1, docLibId))
            .entry.id;
        fileSite2Id = (await apis.admin.nodes.createFile(fileSite2, docLibId))
            .entry.id;
    ...
    it('Consumer - on Search Results - file shared by the user - [C307000]', async () => {
        ...
        expect(await apis.user.nodes.isFileShared(fileSite2Id)).toBe(
            false,
            `${fileSite2} is shared`
        );
    });
