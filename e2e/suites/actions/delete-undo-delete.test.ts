...
describe('Delete and undo delete', () => {
    ...
    describe('on Personal Files', () => {
        ...
        beforeAll(async (done) => {
            ...
            folder1Id = (await apis.user.nodes.createFolder(folder1, parentId))
                .entry.id;
            folder2Id = (await apis.user.nodes.createFolder(folder2, parentId))
                .entry.id;
            folder3Id = (await apis.user.nodes.createFolder(folder3, parentId))
                .entry.id;
            folder4Id = (await apis.user.nodes.createFolder(folder4, parentId))
                .entry.id;
            folder5Id = (await apis.user.nodes.createFolder(folder5, parentId))
                .entry.id;
            folder6Id = (await apis.user.nodes.createFolder(folder6, parentId))
                .entry.id;

            await apis.user.nodes.createFile(file1InFolder, folder1Id);
            fileLocked1Id = (
                await apis.user.nodes.createFile(fileLocked1, folder2Id)
            ).entry.id;
            fileLocked2Id = (
                await apis.user.nodes.createFile(fileLocked2, folder3Id)
            ).entry.id;
            fileLocked3Id = (
                await apis.user.nodes.createFile(fileLocked3, folder4Id)
            ).entry.id;
            fileLocked4Id = (
                await apis.user.nodes.createFile(fileLocked4, folder5Id)
            ).entry.id;
        ...
        it('delete a file and check notification - [C217125]', async () => {
            ...
            expect(await dataTable.isItemPresent(file1)).toBe(
                false,
                'Item was not removed from list'
            );
            items--;
            expect(await page.pagination.getRange()).toContain(
                `1-${items} of ${items}`
            );
            await page.clickTrash();
            expect(await dataTable.isItemPresent(file1)).toBe(
                true,
                'Item is not in trash'
            );
        });
        ...
        it('delete multiple files and check notification - [C280502]', async () => {
            ...
            expect(await dataTable.isItemPresent(file2)).toBe(
                false,
                `${file2} was not removed from list`
            );
            expect(await dataTable.isItemPresent(file3)).toBe(
                false,
                `${file3} was not removed from list`
            );
            items = items - 2;
            expect(await page.pagination.getRange()).toContain(
                `1-${items} of ${items}`
            ...
            expect(await dataTable.isItemPresent(file2)).toBe(
                true,
                `${file2} is not in trash`
            );
            expect(await dataTable.isItemPresent(file3)).toBe(
                true,
                `${file3} is not in trash`
            );
        });
        ...
        it('delete a folder with content - [C217126]', async () => {
            ...
            expect(await dataTable.isItemPresent(file1InFolder)).toBe(
                false,
                'Item is in trash'
            );
        });
        ...
        it('delete a folder containing locked files - [C217127]', async () => {
            ...
            expect(await dataTable.isItemPresent(fileLocked1)).toBe(
                false,
                'Item is in trash'
            );
        });
        ...
        it('undo delete of file - [C217132]', async () => {
            ...
            expect(await dataTable.isItemPresent(file5)).toBe(
                true,
        ...
        it('undo delete of folder with content - [C280503]', async () => {
            ...
            await toolbar.clickMoreActionsDelete();
            await page.clickSnackBarAction();
            expect(await dataTable.isItemPresent(folder6)).toBe(
                true,
                'Item was not restored'
            );
            expect(await page.pagination.getRange()).toContain(
        ...
        it('undo delete of multiple files - [C280504]', async () => {
            ...
            await toolbar.clickMoreActionsDelete();
            await page.clickSnackBarAction();
            expect(await dataTable.isItemPresent(file6)).toBe(
                true,
                `${file6} was not removed from list`
            );
            expect(await dataTable.isItemPresent(file7)).toBe(
                true,
                `${file7} was not removed from list`
            );
            expect(await page.pagination.getRange()).toContain(
                `1-${items} of ${items}`
            );
    ...
    describe('on Shared Files', () => {
        ...
        beforeAll(async (done) => {
            ...
            sharedFile1Id = (
                await apis.user.nodes.createFile(sharedFile1, parentId)
            ).entry.id;
            sharedFile2Id = (
                await apis.user.nodes.createFile(sharedFile2, parentId)
            ).entry.id;
            sharedFile3Id = (
                await apis.user.nodes.createFile(sharedFile3, parentId)
            ).entry.id;
            sharedFile4Id = (
                await apis.user.nodes.createFile(sharedFile4, parentId)
            ).entry.id;
            sharedFile5Id = (
                await apis.user.nodes.createFile(sharedFile5, parentId)
            ).entry.id;
            sharedFile6Id = (
                await apis.user.nodes.createFile(sharedFile6, parentId)
            ).entry.id;
        ...
        it('delete a file and check notification - [C280316]', async () => {
            ...
            expect(message).toContain(`Undo`);
            expect(await dataTable.isItemPresent(sharedFile1)).toBe(
                false,
                'Item was not removed from list'
            );
            await page.clickTrash();
        ...
        it('delete multiple files and check notification - [C280513]', async () => {
            ...
            expect(message).toContain(`Deleted 2 items`);
            expect(message).toContain(`Undo`);
            expect(await dataTable.isItemPresent(sharedFile2)).toBe(
                false,
                `${sharedFile2} was not removed from list`
            );
            expect(await dataTable.isItemPresent(sharedFile3)).toBe(
                false,
                `${sharedFile3} was not removed from list`
            );
            await page.clickTrash();
            expect(await dataTable.isItemPresent(sharedFile2)).toBe(
                true,
                `${sharedFile2} is not in trash`
            );
            expect(await dataTable.isItemPresent(sharedFile3)).toBe(
                true,
                `${sharedFile3} is not in trash`
            );
        });
        ...
        it('undo delete of file - [C280324]', async () => {
            ...
            await page.clickSnackBarAction();
            await page.clickTrash();
            expect(await dataTable.isItemPresent(sharedFile4)).toBe(
                false,
                'Item was not restored'
            );
        });
        ...
        it('undo delete of multiple files - [C280514]', async () => {
            ...
            await page.clickSnackBarAction();
            await page.clickTrash();
            expect(await dataTable.isItemPresent(sharedFile5)).toBe(
                false,
                `${sharedFile5} was not restored`
            );
            expect(await dataTable.isItemPresent(sharedFile6)).toBe(
                false,
                `${sharedFile6} was not restored`
            );
        });
    });
    ...
    describe('on Favorites', () => {
        ...
        beforeAll(async (done) => {
            ...
            favFile1Id = (await apis.user.nodes.createFile(favFile1, parentId))
                .entry.id;
            favFile2Id = (await apis.user.nodes.createFile(favFile2, parentId))
                .entry.id;
            favFile3Id = (await apis.user.nodes.createFile(favFile3, parentId))
                .entry.id;
            favFile4Id = (await apis.user.nodes.createFile(favFile4, parentId))
                .entry.id;
            favFile5Id = (await apis.user.nodes.createFile(favFile5, parentId))
                .entry.id;
            favFile6Id = (await apis.user.nodes.createFile(favFile6, parentId))
                .entry.id;
            favFile7Id = (await apis.user.nodes.createFile(favFile7, parentId))
                .entry.id;

            favFolder1Id = (
                await apis.user.nodes.createFolder(favFolder1, parentId)
            ).entry.id;
            favFolder2Id = (
                await apis.user.nodes.createFolder(favFolder2, parentId)
            ).entry.id;
            favFolder3Id = (
                await apis.user.nodes.createFolder(favFolder3, parentId)
            ).entry.id;
            favFolder4Id = (
                await apis.user.nodes.createFolder(favFolder4, parentId)
            ).entry.id;
            favFolder5Id = (
                await apis.user.nodes.createFolder(favFolder5, parentId)
            ).entry.id;
            favFolder6Id = (
                await apis.user.nodes.createFolder(favFolder6, parentId)
            ).entry.id;

            await apis.user.nodes.createFile(file1InFolder, favFolder1Id);
            fileLocked1Id = (
                await apis.user.nodes.createFile(fileLocked1, favFolder2Id)
            ).entry.id;
            fileLocked2Id = (
                await apis.user.nodes.createFile(fileLocked2, favFolder3Id)
            ).entry.id;
            fileLocked3Id = (
                await apis.user.nodes.createFile(fileLocked3, favFolder4Id)
            ).entry.id;
            fileLocked4Id = (
                await apis.user.nodes.createFile(fileLocked4, favFolder5Id)
            ).entry.id;
        ...
        it('delete a file and check notification - [C280516]', async () => {
            ...
            expect(message).toContain(`Undo`);
            expect(await dataTable.isItemPresent(favFile1)).toBe(
                false,
                'Item was not removed from list'
            );
            items--;
        ...
        it('delete multiple files and check notification - [C280517]', async () => {
            ...
            expect(message).toContain(`Deleted 2 items`);
            expect(message).toContain(`Undo`);
            expect(await dataTable.isItemPresent(favFile2)).toBe(
                false,
                `${favFile2} was not removed from list`
            );
            expect(await dataTable.isItemPresent(favFile3)).toBe(
                false,
                `${favFile3} was not removed from list`
            );
            items = items - 2;
            expect(await page.pagination.getRange()).toContain(
                `1-${items} of ${items}`
            ...
            expect(await dataTable.isItemPresent(favFile2)).toBe(
                true,
                `${favFile2} is not in trash`
            );
            expect(await dataTable.isItemPresent(favFile3)).toBe(
                true,
                `${favFile3} is not in trash`
            );
        });
        ...
        it('delete a folder with content - [C280518]', async () => {
            ...
            await toolbar.clickMoreActionsDelete();
            expect(await dataTable.isItemPresent(favFolder1)).toBe(
                false,
                'Item was not removed from list'
            );
            items--;
        ...
        it('delete a folder containing locked files - [C280519]', async () => {
            ...
            expect(await dataTable.isItemPresent(favFolder2)).toBe(
                true,
                'Item was removed from list'
            );
            await page.clickTrash();
        ...
        it('undo delete of file - [C280524]', async () => {
            ...
            await toolbar.clickMoreActionsDelete();
            await page.clickSnackBarAction();
            expect(await dataTable.isItemPresent(favFile5)).toBe(
                true,
                'Item was not restored'
            );
            expect(await page.pagination.getRange()).toContain(
        ...
        it('undo delete of folder with content - [C280526]', async () => {
            ...
            await toolbar.clickMoreActionsDelete();
            await page.clickSnackBarAction();
            expect(await dataTable.isItemPresent(favFolder6)).toBe(
                true,
                'Item was not restored'
            );
            expect(await page.pagination.getRange()).toContain(
        ...
        it('undo delete of multiple files - [C280525]', async () => {
            ...
            await toolbar.clickMoreActionsDelete();
            await page.clickSnackBarAction();
            expect(await dataTable.isItemPresent(favFile6)).toBe(
                true,
                `${favFile6} was not removed from list`
            );
            expect(await dataTable.isItemPresent(favFile7)).toBe(
                true,
                `${favFile7} was not removed from list`
            );
            expect(await page.pagination.getRange()).toContain(
                `1-${items} of ${items}`
            );
    ...
    describe('on Recent Files', () => {
        ...
        it('delete a file and check notification - [C280528]', async () => {
            ...
            expect(message).toContain(`Undo`);
            expect(await dataTable.isItemPresent(recentFile1)).toBe(
                false,
                'Item was not removed from list'
            );
            await page.clickTrash();
        ...
        it('delete multiple files and check notification - [C280529]', async () => {
            ...
            expect(message).toContain(`Deleted 2 items`);
            expect(message).toContain(`Undo`);
            expect(await dataTable.isItemPresent(recentFile2)).toBe(
                false,
                `${recentFile2} was not removed from list`
            );
            expect(await dataTable.isItemPresent(recentFile3)).toBe(
                false,
                `${recentFile3} was not removed from list`
            );
            await page.clickTrash();
            expect(await dataTable.isItemPresent(recentFile2)).toBe(
                true,
                `${recentFile2} is not in trash`
            );
            expect(await dataTable.isItemPresent(recentFile3)).toBe(
                true,
                `${recentFile3} is not in trash`
            );
        });
        ...
        it('undo delete of file - [C280536]', async () => {
            ...
            await page.clickSnackBarAction();
            await page.clickTrash();
            expect(await dataTable.isItemPresent(recentFile4)).toBe(
                false,
                'Item is in Trash'
            );
        });
        ...
        it('undo delete of multiple files - [C280537]', async () => {
            ...
            await page.clickSnackBarAction();
            await page.clickTrash();
            expect(await dataTable.isItemPresent(recentFile5)).toBe(
                false,
                `${recentFile5} is in Trash`
            );
            expect(await dataTable.isItemPresent(recentFile6)).toBe(
                false,
                `${recentFile6} is in Trash`
            );
        });
    });
});
