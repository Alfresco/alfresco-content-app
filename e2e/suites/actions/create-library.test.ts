...
describe('Create library', () => {
  ...
  it('[C280025] Create a public library', async () => {
    ...
    expect(await repo.sites.getVisibility(site1Name)).toEqual(SITE_VISIBILITY.PUBLIC);
  });
  ...
  it('[C289880] Create a moderated library', async () => {
    ...
    expect(await repo.sites.getVisibility(site2Name)).toEqual(SITE_VISIBILITY.MODERATED);
  });
  ...
  it('[C289881] Create a private library', async () => {
    ...
    expect(await repo.sites.getVisibility(site3Name)).toEqual(SITE_VISIBILITY.PRIVATE);
  });
  ...
  it('[C289882] Create a library with a given ID and description', async () => {
    ...
    expect(await repo.sites.getVisibility(site4.id)).toEqual(SITE_VISIBILITY.PUBLIC);
    expect(await repo.sites.getDescription(site4.id)).toEqual(site4.description);
  });
  ...
  it('[C280030] Create 2 libraries with same name but different IDs', async () => {
    ...
    expect(await repo.sites.getTitle(`${duplicateSite.id}-2`)).toEqual(duplicateSite.name);
  });
});
