...
describe('New menu', () => {
    ...
    it('Actions in Personal Files - [C286524]', async () => {
        ...
        expect(await menu.isUploadFileEnabled()).toBe(
            true,
            'Upload File option not enabled'
        );
        expect(await menu.isUploadFolderEnabled()).toBe(
            true,
            'Upload Folder option not enabled'
        );

        expect(await menu.isCreateFolderEnabled()).toBe(
            true,
            'Create Folder option not enabled'
        );
        expect(await menu.isCreateLibraryEnabled()).toBe(
            true,
            'Create Library option not enabled'
        );

        expect(await menu.isCreateFileFromTemplateEnabled()).toBe(
            true,
            'Create file from template is not enabled'
        );
        expect(await menu.isCreateFolderFromTemplateEnabled()).toBe(
            true,
            'Create folder from template is not enabled'
        );
    });
    ...
    it('Actions in File Libraries - user with enough permissions - [C280393]', async () => {
        ...
        expect(await menu.isUploadFileEnabled()).toBe(
            true,
            'Upload file is not enabled in File Libraries'
        );
        expect(await menu.isUploadFolderEnabled()).toBe(
            true,
            'Upload folder is not enabled in File Libraries'
        );

        expect(await menu.isCreateFolderEnabled()).toBe(
            true,
            'Create folder is not enabled'
        );
        expect(await menu.isCreateLibraryEnabled()).toBe(
            true,
            'Create Library option not enabled'
    ...
    it('Actions in File Libraries - user without enough permissions - [C280397]', async () => {
        ...
        expect(await menu.isUploadFileEnabled()).toBe(
            false,
            'Upload file is not disabled'
        );
        expect(await menu.isUploadFolderEnabled()).toBe(
            false,
            'Upload folder is not disabled'
        );

        expect(await menu.isCreateFolderEnabled()).toBe(
            false,
            'Create folder is not disabled'
        );
        expect(await menu.isCreateLibraryEnabled()).toBe(
            true,
            'Create Library option not enabled'
        ...
        expect(await menu.isCreateFileFromTemplateEnabled()).toBe(
            false,
            'Create file from template is not disabled'
        );
        expect(await menu.isCreateFolderFromTemplateEnabled()).toBe(
            false,
            'Create folder from template is not disabled'
        );
    });
    ...
    it('Disabled actions tooltips - [C280398]', async () => {
        ...
        expect(tooltip).toContain(
            'Files cannot be uploaded whilst viewing the current items'
        );

        tooltip = await menu.getTooltipForUploadFolder();
        expect(tooltip).toContain(
            'Folders cannot be uploaded whilst viewing the current items'
        );

        tooltip = await menu.getTooltipForCreateFolder();
        expect(tooltip).toContain(
            'Folders cannot be created whilst viewing the current items'
        );

        tooltip = await menu.getTooltipForCreateFileFromTemplate();
        expect(tooltip).toContain(
            'Files cannot be created whilst viewing the current items'
        );
    });
});
