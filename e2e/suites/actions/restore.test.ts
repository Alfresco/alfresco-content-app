...
describe('Restore from Trash', () => {
    ...
    describe('successful restore', () => {
        ...
        it('restore file - [C217177]', async () => {
            ...
            expect(await page.dataTable.isItemPresent(file)).toBe(
                true,
                'Item not displayed in list'
            );
        ...
        it('restore folder - [C280438]', async () => {
            ...
            await page.clickPersonalFilesAndWait();
            expect(await page.dataTable.isItemPresent(folder)).toBe(
                true,
                'Item not displayed in list'
            );
        ...
        it('restore library - [C290104]', async () => {
            ...
            expect(await dataTable.isItemPresent(site)).toBe(
                false,
                `${site} was not removed from list`
            );
            await page.clickFileLibrariesAndWait();
            expect(await page.dataTable.isItemPresent(site)).toBe(
                true,
                `${site} not displayed in list`
            );
        });
        ...
        it('View from notification - [C217181]', async () => {
            ...
            expect(await page.sidenav.isActive('Personal Files')).toBe(
                true,
                'Personal Files sidebar link not active'
            );
    ...
    describe('failure to restore', () => {
        ...
        beforeAll(async (done) => {
            ...
            file1Id1 = (await apis.user.nodes.createFile(file1, folder1Id))
                .entry.id;
            await apis.user.nodes.deleteNodeById(file1Id1, false);
            file1Id2 = (await apis.user.nodes.createFile(file1, folder1Id))
                .entry.id;

            folder2Id = (await apis.user.nodes.createFolder(folder2)).entry.id;
            file2Id = (await apis.user.nodes.createFile(file2, folder2Id)).entry
                .id;
        ...
        it('Restore a file when another file with same name exists on the restore location - [C217178]', async () => {
            ...
            expect(await page.getSnackBarMessage()).toEqual(
                `Can't restore, ${file1} already exists`
            );
        });
        ...
        it('Restore a file when original location no longer exists - [C217179]', async () => {
            ...
            await toolbar.clickRestore();
            expect(await page.getSnackBarMessage()).toEqual(
                `Can't restore ${file2}, the original location no longer exists`
            );
        });
    });
    ...
    describe('Notification on partial success', () => {
        ...
        beforeAll(async (done) => {
            folder1Id = (await apis.user.nodes.createFolder(folder1)).entry.id;
            file1Id = (await apis.user.nodes.createFile(file1, folder1Id)).entry
                .id;
            folder2Id = (await apis.user.nodes.createFolder(folder2)).entry.id;
            file2Id = (await apis.user.nodes.createFile(file2, folder2Id)).entry
                .id;
            ...
            file3Id = (await apis.user.nodes.createFile(file3, folder3Id)).entry
                .id;
            file4Id = (await apis.user.nodes.createFile(file4, folder3Id)).entry
                .id;
            folder4Id = (await apis.user.nodes.createFolder(folder4)).entry.id;
            file5Id = (await apis.user.nodes.createFile(file5, folder4Id)).entry
                .id;
        ...
        it('one failure - [C217183]', async () => {
            ...
            await toolbar.clickRestore();
            expect(await page.getSnackBarMessage()).toEqual(
                `Can't restore ${file1}, the original location no longer exists`
            );
        });
