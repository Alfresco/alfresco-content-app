...
describe('Edit folder', () => {
    ...
    beforeAll(async (done) => {
        ...
        folderSiteToEditId = (
            await apis.user.nodes.createFolder(folderSiteToEdit, docLibUserSite)
        ).entry.id;
        await apis.user.nodes.createFolder(duplicateFolderSite, docLibUserSite);

        folderFavoriteId = (await apis.user.nodes.createFolder(folderFavorite))
            .entry.id;
        folderFavoriteToEditId = (
            await apis.user.nodes.createFolder(folderFavoriteToEdit)
        ).entry.id;
        folderFavoriteDuplicateId = (
            await apis.user.nodes.createFolder(folderFavoriteDuplicate)
        ).entry.id;

        await apis.user.nodes.createFolder(folderSearch);
        folderSearchToEditId = (
            await apis.user.nodes.createFolder(folderSearchToEdit)
        ).entry.id;
    ...
    it('dialog UI defaults - [C216331]', async () => {
        ...
        expect(await editDialog.isUpdateButtonEnabled()).toBe(
            true,
            'upload button is not enabled'
        );
        expect(await editDialog.isCancelButtonEnabled()).toBe(
            true,
    ...
    describe('on Personal Files', () => {
        ...
        it('properties are modified when pressing OK - [C216335]', async (done) => {
            ...
            expect(await dataTable.isItemPresent(folderNameEdited)).toBe(
                true,
                'Folder not displayed'
            );
            const desc = await apis.user.nodes.getNodeDescription(
                folderNameEdited,
                parentId
            );
        ...
        it('with empty folder name - [C216332]', async () => {
            ...
            expect(await editDialog.getValidationMessage()).toMatch(
        ...
        it('with name with special characters - [C216333]', async () => {
            ...
            for (const name of namesWithSpecialChars) {
                ...
                expect(await editDialog.getValidationMessage()).toContain(
        ...
        it('Cancel button - [C216336]', async () => {
            ...
            expect(await editDialog.isDialogOpen()).not.toBe(
                true,
        ...
        it('with duplicate folder name - [C216337]', async () => {
            ...
            expect(await editDialog.isDialogOpen()).toBe(
                true,
        ...
        it('trim ending spaces - [C216338]', async () => {
            ...
            expect(await page.isSnackBarPresent()).not.toBe(
                true,
    ...
    describe('on Favorites', () => {
        ...
        it('properties are modified when pressing OK - [C280384]', async (done) => {
            ...
            const desc = await apis.user.nodes.getNodeProperty(
                folderFavoriteToEditId,
                'cm:description'
            );
            expect(desc).toEqual(folderDescriptionEdited);
    ...
    describe('on Search Results', () => {
        ...
        it('properties are modified when pressing OK - [C306947]', async () => {
            ...
            await page.refresh();
            expect(await dataTable.isItemPresent(folderNameEdited2)).toBe(
                true,
                'Folder not displayed'
            );
            const desc = await apis.user.nodes.getNodeProperty(
