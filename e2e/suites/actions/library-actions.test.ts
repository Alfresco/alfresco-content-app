...
describe('Library actions', () => {
    ...
    beforeAll(async (done) => {
        ...
        await apis.admin.sites.createSite(
        ...
        await apis.user.queries.waitForSites('site-public-search', {
            expect: 5,
        });
        await apis.user.queries.waitForSites('site-moderated-search', {
            expect: 2,
        });
    ...
    describe('Join a public library', () => {
        ...
        it('from Favorite Libraries - [C290105]', async () => {
            ...
            expect(await dataTable.getLibraryRole(sitePublic1Admin)).toEqual(
                'Consumer'
            );
        });
        ...
        it('from Search Results - [C306959]', async () => {
            ...
            expect(
                await dataTable.getLibraryRole(siteSearchPublic1Admin)
            ).toEqual('Consumer');
        });
    });
    ...
    describe('Join a moderated library', () => {
        beforeAll(async (done) => {
            ...
            await apis.user.queries.waitForSites(siteSearchModerated1Admin, {
                expect: 1,
            });
            done();
        });
        ...
        it('from Favorite Libraries - [C290109]', async () => {
            ...
            const hasJoinRequest = await apis.user.sites.hasMembershipRequest(
                siteModerated1Admin
            );
            expect(hasJoinRequest).toBe(
                true,
                `Join request does not exist on ${siteModerated1Admin}`
            );
        });
        ...
        it('from Search Results - [C306960]', async () => {
            ...
            expect(
                await dataTable.getLibraryRole(siteSearchModerated1Admin)
            ).toEqual('');
            const hasJoinRequest = await apis.user.sites.hasMembershipRequest(
                siteSearchModerated1Admin
            );
            expect(hasJoinRequest).toBe(
                true,
                `Join request does not exist on ${siteSearchModerated1Admin}`
            );
        });
    });
    ...
    describe('Leave a library', () => {
        beforeAll(async (done) => {
            ...
            await apis.admin.sites.addSiteMember(
                ...
                username,
                SITE_ROLES.SITE_CONTRIBUTOR.ROLE
            );
            await apis.admin.sites.addSiteMember(
                sitePublic4Admin,
                username,
            ...
            await apis.user.queries.waitForSites(siteSearchPublic2Admin, {
                expect: 1,
            });
            done();
        });
        ...
        it('from My Libraries - [C290106]', async () => {
            ...
            expect(await dataTable.isItemPresent(sitePublic2Admin)).toBe(
                false,
                `${sitePublic2Admin} is displayed`
            );
        });
        ...
        it('from Favorite Libraries - [C290110]', async () => {
            ...
            expect(await dataTable.isItemPresent(sitePublic3Admin)).toBe(
                true,
                `${sitePublic3Admin} is not displayed`
            );
        });
        ...
        it('Confirmation dialog UI - [C290136]', async () => {
            ...
            expect(await confirmDialog.getTitle()).toContain(
                'Leave this library?'
            );
            expect(await confirmDialog.getText()).toContain(
                'Leaving will remove your access.'
            );
            expect(await confirmDialog.isOkEnabled()).toBe(
                true,
                'OK button is not enabled'
            );
            expect(await confirmDialog.isCancelEnabled()).toBe(
                true,
        ...
        it('Cancel Leave Library - [C290111]', async () => {
            ...
            expect(await dataTable.isItemPresent(sitePublic5Admin)).toBe(
                true,
                `${sitePublic5Admin} was deleted`
            );
        });
    ...
    describe('Cancel join', () => {
        beforeAll(async (done) => {
            ...
            await apis.user.queries.waitForSites(siteSearchModerated2Admin, {
                expect: 1,
            });
            done();
        });
        ...
        it('from Favorite Libraries - [C290108]', async () => {
            ...
            expect(await page.getSnackBarMessage()).toEqual(
                `Canceled the request to join the library`
            );
            ...
            expect(hasJoinRequest).toBe(
                false,
                `Join request exists on ${siteModerated2Admin}`
            );
        });
        ...
        it('from Search Results - [C306962]', async () => {
            ...
            expect(hasJoinRequest).toBe(
                false,
                `Join request exists on ${siteSearchModerated2Admin}`
            );
        });
    });
    ...
    describe('Mark library as favorite', () => {
        beforeAll(async (done) => {
            ...
            await apis.user.queries.waitForSites(siteSearchPublic3Admin, {
                expect: 1,
            });
            done();
        });
        ...
        it('from My Libraries - [C289974]', async () => {
            ...
            ).toBe(true, `${sitePublic6Admin} not favorite`);
        });
        ...
        it('from on Search Results - [C306963]', async () => {
            ...
            ).toBe(true, `${siteSearchPublic3Admin} not favorite`);
        });
    });
    ...
    describe('Remove library from favorites', () => {
        beforeAll(async (done) => {
            ...
            await apis.user.queries.waitForSites(siteSearchPublic4Admin, {
                expect: 1,
            });
            done();
        });
        ...
        it('from My Libraries - [C289975]', async () => {
            ...
            ).toBe(false, `${sitePublic7Admin} still favorite`);
        });
        ...
        it('from Favorite Libraries - [C289976]', async () => {
            ...
            expect(await dataTable.isItemPresent(sitePublic8Admin)).toBe(
                false,
                `${sitePublic8Admin} is displayed`
            );
            expect(
                await apis.user.favorites.isFavoriteWithRetry(
                    sitePublic8Admin,
            ...
            ).toBe(false, `${sitePublic8Admin} still favorite`);
        });
        ...
        it('from Search Results - [C306964]', async () => {
            ...
            ).toBe(false, `${siteSearchPublic4Admin} still favorite`);
        });
    });
    ...
    describe('Delete a library', () => {
        beforeAll(async (done) => {
            ...
            await apis.user.queries.waitForSites(siteSearchForDelete, {
                expect: 1,
            });
            done();
        });
        ...
        it('from My Libraries - [C289988]', async () => {
            ...
            expect(await dataTable.isItemPresent(siteForDelete1)).toBe(
                false,
                `${siteForDelete1} still displayed`
            );
        });
        ...
        it('from Favorite Libraries - [C289991]', async () => {
            ...
            expect(await page.getSnackBarMessage()).toEqual(`Library deleted`);
            expect(await dataTable.isItemPresent(siteForDelete2)).toBe(
                false,
                `${siteForDelete2} still displayed`
            );
        });
        ...
        it('from Search Results - [C306965]', async () => {
            ...
            expect(await page.getSnackBarMessage()).toEqual(`Library deleted`);
            expect(await dataTable.isItemPresent(siteSearchForDelete)).toBe(
                false,
                `${siteSearchForDelete} still displayed`
            );
        });
    });
});
