...
describe('Edit offline', () => {
    ...
    describe('on Personal Files', () => {
        beforeAll(async () => {
            parentPFId = (await apis.user.nodes.createFolder(parentPF)).entry
                .id;
        ...
        it('[C297538] - File is locked and downloaded when clicking Edit Offline', async () => {
            ...
            expect(await Utils.fileExistsOnOS(file1)).toBe(
                true,
                'File not found in download location'
            );
            expect(await apis.user.nodes.isFileLockedWrite(file1Id)).toBe(
                true,
                `${file1} is not locked`
            );
        });
        ...
        it('[C297539] - Lock information is displayed', async () => {
            expect(await dataTable.isItemPresent(fileLocked2)).toBe(
                true,
                `${fileLocked2} is not displayed`
            );
            expect(await dataTable.hasLockIcon(fileLocked2)).toBe(
                true,
                `${fileLocked2} does not have a lock icon`
            );
        ...
        it('[C297540] - Cancel Editing unlocks the file', async () => {
            ...
            expect(await apis.user.nodes.isFileLockedWrite(fileLockedId)).toBe(
                false,
                `${fileLocked} is still locked`
            );
            expect(await dataTable.hasLockIcon(fileLocked)).toBe(
                false,
                `${fileLocked} has a lock icon`
            );
        });
    });
    ...
    describe('on Shared Files', () => {
        beforeAll(async () => {
            parentSFId = (await apis.user.nodes.createFolder(parentSF)).entry
                .id;
        ...
        it('[C306951] - Lock information is displayed', async () => {
            expect(await dataTable.isItemPresent(fileLocked2, parentSF)).toBe(
                true,
                `${fileLocked2} is not displayed`
            );
            expect(await dataTable.hasLockIcon(fileLocked2, parentSF)).toBe(
                true,
                `${fileLocked2} does not have a lock icon`
            );
            expect(
                await dataTable.getLockOwner(fileLocked2, parentSF)
            ).toContain(
                username,
                `${fileLocked2} does not have correct lock owner info`
            );
        ...
        it('[C306952] - Cancel Editing unlocks the file', async () => {
            ...
            expect(await dataTable.hasLockIcon(fileLocked, parentSF)).toBe(
                false,
                `${fileLocked} has a lock icon`
            );
        });
    ...
    describe('on Recent Files', () => {
        beforeAll(async () => {
            parentRFId = (await apis.user.nodes.createFolder(parentRF)).entry
                .id;
        ...
        it('[C297542] -  Lock information is displayed', async () => {
            ...
            expect(await dataTable.hasLockIcon(fileLocked2, parentRF)).toBe(
                true,
                `${fileLocked2} does not have a lock icon`
            );
            expect(
                await dataTable.getLockOwner(fileLocked2, parentRF)
            ).toContain(
                username,
                `${fileLocked2} does not have correct lock owner info`
            );
        ...
        it('[C297543] - Cancel Editing unlocks the file', async () => {
            ...
            expect(await dataTable.hasLockIcon(fileLocked, parentRF)).toBe(
                false,
                `${fileLocked} has a lock icon`
            );
        });
    ...
    describe('on Favorite Files', () => {
        beforeAll(async () => {
            parentFavId = (await apis.user.nodes.createFolder(parentFav)).entry
                .id;
    ...
    describe('on Search Results', () => {
        ...
        it('[C306953] - File is locked and downloaded when clicking Edit Offline', async () => {
            ...
            expect(await Utils.fileExistsOnOS(fileSearch1)).toBe(
                true,
                'File not found in download location'
            );
            expect(await apis.user.nodes.isFileLockedWrite(fileSearch1Id)).toBe(
                true,
                `${fileSearch1} is not locked`
            );
        });

        it('[C306954] - Lock information is displayed', async () => {
            expect(
                await dataTable.isItemPresent(fileSearchLocked2, parentSearch)
            ).toBe(true, `${fileSearchLocked2} is not displayed`);
            expect(
                await dataTable.hasLockIcon(fileSearchLocked2, parentSearch)
            ).toBe(true, `${fileSearchLocked2} does not have a lock icon`);
        ...
        it('Cancel Editing unlocks the file - [C306955]', async () => {
            ...
            expect(
                await apis.user.nodes.isFileLockedWrite(fileSearchLockedId)
            ).toBe(false, `${fileSearchLocked} is still locked`);
            expect(
                await dataTable.hasLockIcon(fileSearchLocked, parentSearch)
            ).toBe(false, `${fileSearchLocked} has a lock icon`);
        });
    });
});
