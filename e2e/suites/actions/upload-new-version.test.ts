...
describe('Upload new version', () => {
    ...
    const nameConflictMessage =
        'New version not uploaded, another file with the same name already exists';
    ...
    beforeAll(async (done) => {
        ...
        parentSearchId = (await apis.user.nodes.createFolder(parentSearch))
            .entry.id;
    ...
    describe('on Personal Files', () => {
        beforeAll(async (done) => {
            fileId = (await apis.user.upload.uploadFile(file, parentPFId)).entry
                .id;
            file1Id = (await apis.user.nodes.createFile(file1, parentPFId))
                .entry.id;
            file2Id = (await apis.user.nodes.createFile(file2, parentPFId))
                .entry.id;
            file3Id = (await apis.user.nodes.createFile(file3, parentPFId))
                .entry.id;
            file4Id = (await apis.user.nodes.createFile(file4, parentPFId))
                .entry.id;

            fileLocked1Id = (
                await apis.user.nodes.createFile(fileLocked1, parentPFId)
            ).entry.id;
            fileLocked2Id = (
                await apis.user.nodes.createFile(fileLocked2, parentPFId)
            ).entry.id;
        ...
        it('dialog UI defaults - [C297544]', async () => {
            ...
            expect(await uploadNewVersionDialog.getTitle()).toEqual(
                'Upload New Version'
            );
            expect(await uploadNewVersionDialog.getText()).toContain(
                'What level of changes were made to this version?'
            );
            expect(await uploadNewVersionDialog.isDescriptionDisplayed()).toBe(
                true,
                'Description not displayed'
            );
            expect(await uploadNewVersionDialog.isMinorOptionDisplayed()).toBe(
                true,
                'Minor option not displayed'
            );
            expect(await uploadNewVersionDialog.isMajorOptionDisplayed()).toBe(
                true,
                'Major option not displayed'
            );
            expect(await uploadNewVersionDialog.isCancelButtonEnabled()).toBe(
                true,
                'Cancel button not enabled'
            );
            expect(await uploadNewVersionDialog.isUploadButtonEnabled()).toBe(
                true,
                'Update button not enabled'
            );
        });
        ...
        it('file is updated after uploading a new version - major - [C297545]', async () => {
            ...
            await uploadNewVersionDialog.enterDescription(
                'new major version description'
            );
            await uploadNewVersionDialog.clickUpload();
            await uploadNewVersionDialog.waitForDialogToClose();

            expect(await dataTable.isItemPresent(fileToUpload1)).toBe(
                true,
                'File not updated'
            );
            expect(await apis.user.nodes.getFileVersionType(file1Id)).toEqual(
                'MAJOR',
                'File has incorrect version type'
            );
            expect(await apis.user.nodes.getFileVersionLabel(file1Id)).toEqual(
                '2.0',
        ...
        it('file is updated after uploading a new version - minor - [C297546]', async () => {
            ...
            await uploadNewVersionDialog.enterDescription(
                'new minor version description'
            );
            await uploadNewVersionDialog.clickUpload();
            await uploadNewVersionDialog.waitForDialogToClose();

            expect(await dataTable.isItemPresent(fileToUpload2)).toBe(
                true,
                'File not updated'
            );
            expect(await apis.user.nodes.getFileVersionType(file2Id)).toEqual(
                'MINOR',
                'File has incorrect version type'
            );
            expect(await apis.user.nodes.getFileVersionLabel(file2Id)).toEqual(
                '1.1',
                'File has incorrect version label'
            );
        });
        ...
        it('file is not updated when clicking Cancel - [C297547]', async () => {
            ...
            expect(await apis.user.nodes.getFileVersionType(file3Id)).toEqual(
                'MAJOR',
                'File has incorrect version type'
            );
            expect(await apis.user.nodes.getFileVersionLabel(file3Id)).toEqual(
                '1.0',
                'File has incorrect version label'
            );
        });
        ...
        it('upload new version fails when new file name already exists - [C297548]', async () => {
            ...
            expect(await apis.user.nodes.getFileVersionType(file4Id)).toEqual(
                'MAJOR',
                'File has incorrect version type'
            );
            expect(await apis.user.nodes.getFileVersionLabel(file4Id)).toEqual(
                '1.0',
                'File has incorrect version label'
            );
        });
        ...
        it('file is unlocked after uploading a new version - [C297549]', async () => {
            ...
            expect(await dataTable.isItemPresent(fileToUpload4)).toBe(
                true,
                'File name was not changed'
            );
            expect(await apis.user.nodes.isFileLockedWrite(fileLocked1Id)).toBe(
                false,
                `${fileLocked1} is still locked`
            );
            expect(
                await apis.user.nodes.getFileVersionType(fileLocked1Id)
            ).toEqual('MINOR', 'File has incorrect version type');
            expect(
                await apis.user.nodes.getFileVersionLabel(fileLocked1Id)
            ).toEqual('1.1', 'File has incorrect version label');
        });
        ...
        it('file remains locked after canceling of uploading a new version - [C297550]', async () => {
            ...
            expect(await dataTable.isItemPresent(fileToUpload5)).toBe(
                false,
                'File was updated'
            );
            expect(await dataTable.isItemPresent(fileLocked2)).toBe(
                true,
                'File not displayed'
            );
            expect(await apis.user.nodes.isFileLockedWrite(fileLocked2Id)).toBe(
                true,
                `${fileLocked2} was unlocked`
            );
        });
    });
    ...
    describe('on Shared Files', () => {
        beforeAll(async (done) => {
            fileId = (await apis.user.upload.uploadFile(file, parentSFId)).entry
                .id;
            file1Id = (await apis.user.nodes.createFile(file1, parentSFId))
                .entry.id;
            file2Id = (await apis.user.nodes.createFile(file2, parentSFId))
                .entry.id;
            file3Id = (await apis.user.nodes.createFile(file3, parentSFId))
                .entry.id;
            file4Id = (await apis.user.nodes.createFile(file4, parentSFId))
                .entry.id;

            fileLocked1Id = (
                await apis.user.nodes.createFile(fileLocked1, parentSFId)
            ).entry.id;
            fileLocked2Id = (
                await apis.user.nodes.createFile(fileLocked2, parentSFId)
            ).entry.id;
    ...
    describe('on Recent Files', () => {
        beforeAll(async (done) => {
            fileId = (await apis.user.upload.uploadFile(file, parentRFId)).entry
                .id;
            file1Id = (await apis.user.nodes.createFile(file1, parentRFId))
                .entry.id;
            file2Id = (await apis.user.nodes.createFile(file2, parentRFId))
                .entry.id;
            file3Id = (await apis.user.nodes.createFile(file3, parentRFId))
                .entry.id;
            file4Id = (await apis.user.nodes.createFile(file4, parentRFId))
                .entry.id;

            fileLocked1Id = (
                await apis.user.nodes.createFile(fileLocked1, parentRFId)
            ).entry.id;
            fileLocked2Id = (
                await apis.user.nodes.createFile(fileLocked2, parentRFId)
            ).entry.id;
        ...
        it('file is updated after uploading a new version - major - [C297559]', async () => {
            ...
            expect(await dataTable.isItemPresent(fileToUpload1, parentRF)).toBe(
                true,
                'File not updated'
            );
            expect(await apis.user.nodes.getFileVersionType(file1Id)).toEqual(
        ...
        it('file is updated after uploading a new version - minor - [C297560]', async () => {
            ...
            expect(await dataTable.isItemPresent(fileToUpload2, parentRF)).toBe(
                true,
                'File not updated'
            );
            expect(await apis.user.nodes.getFileVersionType(file2Id)).toEqual(
        ...
        it('file is not updated when clicking Cancel - [C297561]', async () => {
            ...
            expect(await dataTable.isItemPresent(file3, parentRF)).toBe(
                true,
                'File was updated'
            );
            expect(await apis.user.nodes.getFileVersionType(file3Id)).toEqual(
        ...
        it('upload new version fails when new file name already exists - [C297562]', async () => {
            ...
            expect(await dataTable.isItemPresent(file4, parentRF)).toBe(
                true,
                'File was updated'
            );
            expect(await apis.user.nodes.getFileVersionType(file4Id)).toEqual(
        ...
        it('file is unlocked after uploading a new version - [C297563]', async () => {
            ...
            expect(await dataTable.isItemPresent(fileToUpload4, parentRF)).toBe(
                true,
                'File name was not changed'
            );
            expect(await apis.user.nodes.isFileLockedWrite(fileLocked1Id)).toBe(
        ...
        it('file remains locked after canceling of uploading a new version - [C297564]', async () => {
            ...
            expect(await dataTable.isItemPresent(fileToUpload5, parentRF)).toBe(
                false,
                'File was updated'
            );
            expect(await dataTable.isItemPresent(fileLocked2, parentRF)).toBe(
                true,
                'File not displayed'
            );
            expect(await apis.user.nodes.isFileLockedWrite(fileLocked2Id)).toBe(
    ...
    describe('on Favorite Files', () => {
        beforeAll(async (done) => {
            fileId = (await apis.user.upload.uploadFile(file, parentFavId))
                .entry.id;
            file1Id = (await apis.user.nodes.createFile(file1, parentFavId))
                .entry.id;
            file2Id = (await apis.user.nodes.createFile(file2, parentFavId))
                .entry.id;
            file3Id = (await apis.user.nodes.createFile(file3, parentFavId))
                .entry.id;
            file4Id = (await apis.user.nodes.createFile(file4, parentFavId))
                .entry.id;

            fileLocked1Id = (
                await apis.user.nodes.createFile(fileLocked1, parentFavId)
            ).entry.id;
            fileLocked2Id = (
                await apis.user.nodes.createFile(fileLocked2, parentFavId)
            ).entry.id;
    ...
    describe('on Search Results', () => {
        beforeAll(async (done) => {
            fileId = (await apis.user.upload.uploadFile(file, parentSearchId))
                .entry.id;
            fileSearch1Id = (
                await apis.user.nodes.createFile(fileSearch1, parentSearchId)
            ).entry.id;
            fileSearch2Id = (
                await apis.user.nodes.createFile(fileSearch2, parentSearchId)
            ).entry.id;
            fileSearch3Id = (
                await apis.user.nodes.createFile(fileSearch3, parentSearchId)
            ).entry.id;
            fileSearch4Id = (
                await apis.user.nodes.createFile(fileSearch4, parentSearchId)
            ).entry.id;
        ...
        it('file is updated after uploading a new version - major - [C307004]', async () => {
            ...
            expect(
                await apis.user.nodes.getFileVersionType(fileSearch1Id)
            ).toEqual('MAJOR', 'File has incorrect version type');
            expect(
                await apis.user.nodes.getFileVersionLabel(fileSearch1Id)
            ).toEqual('2.0', 'File has incorrect version label');
        });
        ...
        it('file is updated after uploading a new version - minor - [C307005]', async () => {
            ...
            expect(
                await apis.user.nodes.getFileVersionType(fileSearch2Id)
            ).toEqual('MINOR', 'File has incorrect version type');
            expect(
                await apis.user.nodes.getFileVersionLabel(fileSearch2Id)
            ).toEqual('1.1', 'File has incorrect version label');
        });
        ...
        it('file is not updated when clicking Cancel - [C307006]', async () => {
            ...
            expect(
                await dataTable.isItemPresent(fileSearch3, parentSearch)
            ).toBe(true, 'File was updated');
            expect(
                await apis.user.nodes.getFileVersionType(fileSearch3Id)
            ).toEqual('MAJOR', 'File has incorrect version type');
            expect(
                await apis.user.nodes.getFileVersionLabel(fileSearch3Id)
            ).toEqual('1.0', 'File has incorrect version label');
        });
        ...
        it('upload new version fails when new file name already exists - [C307007]', async () => {
            ...
            expect(
                await dataTable.isItemPresent(fileSearch4, parentSearch)
            ).toBe(true, 'File was updated');
            expect(
                await apis.user.nodes.getFileVersionType(fileSearch4Id)
            ).toEqual('MAJOR', 'File has incorrect version type');
            expect(
                await apis.user.nodes.getFileVersionLabel(fileSearch4Id)
            ).toEqual('1.0', 'File has incorrect version label');
        });
        ...
        it('file is unlocked after uploading a new version - [C307008]', async () => {
            ...
            expect(
                await apis.user.nodes.isFileLockedWrite(fileLockedSearch1Id)
            ).toBe(false, `${fileLockedSearch1} is still locked`);
            expect(
                await apis.user.nodes.getFileVersionType(fileLockedSearch1Id)
            ).toEqual('MINOR', 'File has incorrect version type');
            expect(
                await apis.user.nodes.getFileVersionLabel(fileLockedSearch1Id)
            ).toEqual('1.1', 'File has incorrect version label');
        });
        ...
        it('file remains locked after canceling of uploading a new version - [C307009]', async () => {
            ...
            expect(
                await dataTable.isItemPresent(fileToUpload5, parentSearch)
            ).toBe(false, 'File was updated');
            expect(
                await dataTable.isItemPresent(fileLockedSearch2, parentSearch)
            ).toBe(true, 'File not displayed');
            expect(
                await apis.user.nodes.isFileLockedWrite(fileLockedSearch2Id)
            ).toBe(true, `${fileLockedSearch2} was unlocked`);
        });
    });
});
