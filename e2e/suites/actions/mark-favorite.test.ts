...
describe('Mark items as favorites', () => {
    ...
    let fileSearchNotFav1Id,
        fileSearchNotFav2Id,
        fileSearchNotFav3Id,
        fileSearchNotFav4Id;
    let fileSearchFav1Id,
        fileSearchFav2Id,
        fileSearchFav3Id,
        fileSearchFav4Id,
        folderSearchId;
    ...
    beforeAll(async (done) => {
        ...
        fileFavUIId = (await apis.user.nodes.createFile(fileFavUI, parentId))
            .entry.id;
        fileNotFav1Id = (
            await apis.user.nodes.createFile(fileNotFav1, parentId)
        ).entry.id;
        fileNotFav2Id = (
            await apis.user.nodes.createFile(fileNotFav2, parentId)
        ).entry.id;
        fileNotFav3Id = (
            await apis.user.nodes.createFile(fileNotFav3, parentId)
        ).entry.id;
        fileNotFav4Id = (
            await apis.user.nodes.createFile(fileNotFav4, parentId)
        ).entry.id;
        fileFav1Id = (await apis.user.nodes.createFile(fileFav1, parentId))
            .entry.id;
        fileFav2Id = (await apis.user.nodes.createFile(fileFav2, parentId))
            .entry.id;
        fileFav3Id = (await apis.user.nodes.createFile(fileFav3, parentId))
            .entry.id;
        fileFav4Id = (await apis.user.nodes.createFile(fileFav4, parentId))
            .entry.id;
        folderId = (await apis.user.nodes.createFolder(folder, parentId)).entry
            .id;

        fileSearchNotFav1Id = (
            await apis.user.nodes.createFile(fileSearchNotFav1, parentId)
        ).entry.id;
        fileSearchNotFav2Id = (
            await apis.user.nodes.createFile(fileSearchNotFav2, parentId)
        ).entry.id;
        fileSearchNotFav3Id = (
            await apis.user.nodes.createFile(fileSearchNotFav3, parentId)
        ).entry.id;
        fileSearchNotFav4Id = (
            await apis.user.nodes.createFile(fileSearchNotFav4, parentId)
        ).entry.id;
        fileSearchFav1Id = (
            await apis.user.nodes.createFile(fileSearchFav1, parentId)
        ).entry.id;
        fileSearchFav2Id = (
            await apis.user.nodes.createFile(fileSearchFav2, parentId)
        ).entry.id;
        fileSearchFav3Id = (
            await apis.user.nodes.createFile(fileSearchFav3, parentId)
        ).entry.id;
        fileSearchFav4Id = (
            await apis.user.nodes.createFile(fileSearchFav4, parentId)
        ).entry.id;
        folderSearchId = (
            await apis.user.nodes.createFolder(folderSearch, parentId)
        ).entry.id;
    ...
    describe('on Personal Files', () => {
        afterAll(async (done) => {
            try {
                ...
                await apis.user.favorites.removeFavoritesByIds([
                    fileNotFav1Id,
                    fileNotFav2Id,
                    fileNotFav3Id,
                    fileNotFav4Id,
        ...
        it('Favorite action has empty star icon for an item not marked as favorite - [C217186]', async () => {
            ...
            expect(await toolbar.menu.getItemIconText('Favorite')).toEqual(
                'star_border'
            );
        });
        ...
        it('Favorite action has full star icon for items marked as favorite - [C217188]', async () => {
            ...
            expect(
                await toolbar.menu.getItemIconText('Remove Favorite')
            ).toEqual('star');
        });
        ...
        it('favorite a file - [C217189]', async () => {
            ...
            expect(
                await apis.user.favorites.isFavoriteWithRetry(fileNotFav1Id, {
                    expect: true,
                })
            ).toBe(true, `${fileNotFav1} not marked as favorite`);
        });
        ...
        it('favorite a folder - [C280390]', async () => {
            ...
            expect(
                await apis.user.favorites.isFavoriteWithRetry(folderId, {
                    expect: true,
                })
            ).toBe(true, `${folder} not marked as favorite`);
        });
        ...
        it('unfavorite an item - [C217190]', async () => {
            ...
            expect(
                await apis.user.favorites.isFavoriteWithRetry(fileFav1Id, {
                    expect: false,
                })
            ).toBe(false, `${fileFav1} is marked as favorite`);
        });
        ...
        it('favorite multiple items - all unfavorite - [C217192]', async () => {
            ...
            expect(
                await apis.user.favorites.isFavoriteWithRetry(fileNotFav2Id, {
                    expect: true,
                })
            ).toBe(true, `${fileNotFav2} not marked as favorite`);
            expect(
                await apis.user.favorites.isFavoriteWithRetry(fileNotFav3Id, {
                    expect: true,
                })
            ).toBe(true, `${fileNotFav3} not marked as favorite`);
        });
        ...
        it('favorite multiple items - some favorite and some unfavorite - [C217194]', async () => {
            ...
            expect(
                await apis.user.favorites.isFavoriteWithRetry(fileNotFav4Id, {
                    expect: true,
                })
            ).toBe(true, `${fileNotFav4} not marked as favorite`);
            expect(
                await apis.user.favorites.isFavoriteWithRetry(fileFav2Id, {
                    expect: true,
                })
            ).toBe(true, `${fileFav2} not marked as favorite`);
        });
        ...
        it('unfavorite multiple items - [C217193]', async () => {
            ...
            expect(
                await apis.user.favorites.isFavoriteWithRetry(fileFav3Id, {
                    expect: false,
                })
            ).toBe(false, `${fileFav3} marked as favorite`);
            expect(
                await apis.user.favorites.isFavoriteWithRetry(fileFav4Id, {
                    expect: false,
                })
            ).toBe(false, `${fileFav4} marked as favorite`);
        });
    });
    ...
    describe('on Favorites', () => {
        ...
        it('unfavorite an item - [C280368]', async () => {
            ...
            expect(await dataTable.isItemPresent(fileFav1)).toBe(
                false,
                'item still displayed'
            );
        });
        ...
        it('unfavorite multiple items - [C280374]', async () => {
            ...
            expect(await dataTable.isItemPresent(fileFav3)).toBe(
                false,
                'file3 still displayed'
            );
            expect(await dataTable.isItemPresent(fileFav4)).toBe(
                false,
                'file4 still displayed'
            );
        });
    ...
    describe('on Search Results', () => {
        ...
        it('favorite a file - [C306966]', async () => {
            ...
            expect(
                await apis.user.favorites.isFavoriteWithRetry(
                    fileSearchNotFav1Id,
                    { expect: true }
                )
            ).toBe(true, `${fileSearchNotFav1} not marked as favorite`);
        });
        ...
        it('favorite a folder - [C306971]', async () => {
            ...
            expect(
                await apis.user.favorites.isFavoriteWithRetry(folderSearchId, {
                    expect: true,
                })
            ).toBe(true, `${folderSearch} not marked as favorite`);
        });
        ...
        it('unfavorite an item - [C306967]', async () => {
            ...
            ).toBe(false, `${fileSearchFav1} is marked as favorite`);
        });
        ...
        it('favorite multiple items - all unfavorite - [C306968]', async () => {
            ...
            ).toBe(true, `${fileSearchNotFav2} not marked as favorite`);
            expect(
                await apis.user.favorites.isFavoriteWithRetry(
                    fileSearchNotFav3Id,
                    { expect: true }
                )
            ).toBe(true, `${fileSearchNotFav3} not marked as favorite`);
        });
        ...
        it('favorite multiple items - some favorite and some unfavorite - [C306970]', async () => {
            ...
            ).toBe(true, `${fileSearchNotFav4} not marked as favorite`);
            expect(
                await apis.user.favorites.isFavoriteWithRetry(
                    fileSearchFav2Id,
                    { expect: true }
                )
            ).toBe(true, `${fileSearchFav2} not marked as favorite`);
        });
        ...
        it('unfavorite multiple items - [C306969]', async () => {
            ...
            ).toBe(false, `${fileSearchFav3} marked as favorite`);
            expect(
                await apis.user.favorites.isFavoriteWithRetry(
                    fileSearchFav4Id,
                    { expect: false }
                )
            ).toBe(false, `${fileSearchFav4} marked as favorite`);
        });
    });
    ...
    describe('on File Libraries', () => {
        ...
        beforeAll(async (done) => {
            ...
            folderSiteId = (
                await apis.user.nodes.createFolder(folderSite, docLibId)
            ).entry.id;
            fileSiteNotFav1Id = (
                await apis.user.nodes.createFile(fileSiteNotFav1, folderSiteId)
            ).entry.id;
            fileSiteNotFav2Id = (
                await apis.user.nodes.createFile(fileSiteNotFav2, folderSiteId)
            ).entry.id;
            fileSiteNotFav3Id = (
                await apis.user.nodes.createFile(fileSiteNotFav3, folderSiteId)
            ).entry.id;
            fileSiteNotFav4Id = (
                await apis.user.nodes.createFile(fileSiteNotFav4, folderSiteId)
            ).entry.id;
            fileSiteFav1Id = (
                await apis.user.nodes.createFile(fileSiteFav1, folderSiteId)
            ).entry.id;
            fileSiteFav2Id = (
                await apis.user.nodes.createFile(fileSiteFav2, folderSiteId)
            ).entry.id;
            fileSiteFav3Id = (
                await apis.user.nodes.createFile(fileSiteFav3, folderSiteId)
            ).entry.id;
            fileSiteFav4Id = (
                await apis.user.nodes.createFile(fileSiteFav4, folderSiteId)
            ).entry.id;
            ...
            await apis.user.favorites.isFavoriteWithRetry(fileSiteFav1Id, {
                expect: true,
            });
            await apis.user.favorites.isFavoriteWithRetry(fileSiteFav2Id, {
                expect: true,
            });
            await apis.user.favorites.isFavoriteWithRetry(fileSiteFav3Id, {
                expect: true,
            });
            await apis.user.favorites.isFavoriteWithRetry(fileSiteFav4Id, {
                expect: true,
            });
        ...
        it('Favorite a folder - [C280391]', async () => {
            ...
            expect(
                await apis.user.favorites.isFavoriteWithRetry(folderSiteId, {
                    expect: true,
                })
            ).toBe(true, `${folderSite} not marked as favorite`);
        });
        ...
        it('Favorite a file - [C280342]', async () => {
            ...
            ).toBe(true, `${fileSiteNotFav1} not marked as favorite`);
        });
        ...
        it('Unfavorite an item - [C280343]', async () => {
            ...
            ).toBe(false, `${fileSiteFav1} is marked as favorite`);
        });
        ...
        it('Favorite multiple items - all unfavorite - [C280345]', async () => {
            ...
            ).toBe(true, 'item not marked as favorite');
            expect(
                await apis.user.favorites.isFavoriteWithRetry(
                    fileSiteNotFav3Id,
                    { expect: true }
        ...
        it('Unfavorite multiple items - [C280346]', async () => {
            ...
            ).toBe(false, 'item marked as favorite');
            expect(
                await apis.user.favorites.isFavoriteWithRetry(fileSiteFav3Id, {
                    expect: false,
                })
