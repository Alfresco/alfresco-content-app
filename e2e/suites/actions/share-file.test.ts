...
describe('Share a file', () => {
    ...
    describe('when logged out', () => {
        ...
        beforeAll(async (done) => {
            file6Id = (await apis.user.nodes.createFile(file6, parentId)).entry
                .id;
            const sharedId = (await apis.user.shared.shareFileById(file6Id))
                .entry.id;
        ...
        it('A non-logged user can download the shared file from the viewer - [C286326]', async () => {
            ...
            expect(await Utils.fileExistsOnOS(file6)).toBe(
                true,
    ...
    describe('when logged in', () => {
        ...
        describe('from Personal Files', () => {
            beforeAll(async (done) => {
                file1Id = (await apis.user.nodes.createFile(file1, parentId))
                    .entry.id;
                file2Id = (await apis.user.nodes.createFile(file2, parentId))
                    .entry.id;
                file3Id = (await apis.user.nodes.createFile(file3, parentId))
                    .entry.id;
                file4Id = (await apis.user.nodes.createFile(file4, parentId))
                    .entry.id;
                file5Id = (await apis.user.nodes.createFile(file5, parentId))
                    .entry.id;
                file6Id = (await apis.user.nodes.createFile(file6, parentId))
                    .entry.id;
                file7Id = (await apis.user.nodes.createFile(file7, parentId))
                    .entry.id;
                file8Id = (await apis.user.nodes.createFile(file8, parentId))
                    .entry.id;
                file9Id = (await apis.user.nodes.createFile(file9, parentId))
                    .entry.id;
            ...
            it('Share dialog default values - [C286327]', async () => {
                ...
                expect(await shareDialog.getInfoText()).toEqual(
                    'Click the link below to copy it to the clipboard.'
                );
                expect(await shareDialog.getLabels().get(0).getText()).toEqual(
                    'Link to share'
                );
                expect(await shareDialog.getLinkUrl()).toContain(
                    shareLinkPreUrl
                );
                expect(await shareDialog.isUrlReadOnly()).toBe(
                    true,
                    'url is not readonly'
                );
                expect(await shareDialog.isShareToggleChecked()).toBe(
                    true,
                    'Share toggle not checked'
                );
                expect(await shareDialog.getLabels().get(1).getText()).toEqual(
                    'Expires on'
                );
                expect(await shareDialog.isExpireToggleEnabled()).toBe(
                    false,
                    'Expire toggle is checked'
                );
                expect(await shareDialog.isCloseEnabled()).toBe(
                    true,
            ...
            it('Share a file - [C286329]', async () => {
                ...
                expect(await apis.user.nodes.isFileShared(file3Id)).toBe(
                    true,
                    `${file3} is not shared`
                );
            ...
            it('Copy shared file URL - [C286330]', async () => {
                ...
                expect(await page.getSnackBarMessage()).toBe(
            ...
            it('Share a file with expiration date - [C286332]', async () => {
                ...
                expect(await shareDialog.isExpireToggleEnabled()).toBe(
                    true,
                    'Expire toggle not checked'
                );
                await shareDialog.openDatetimePicker();
                expect(await shareDialog.dateTimePicker.isCalendarOpen()).toBe(
                    true,
                    'Calendar not opened'
                );
                ...
                const expireDateProperty = await apis.user.nodes.getSharedExpiryDate(
                    file5Id
                );

                expect(Utils.formatDate(expireDateProperty)).toEqual(
                    Utils.formatDate(inputDate)
                );
            });
            ...
            it('Expire date is displayed correctly - [C286337]', async () => {
                ...
                const expireProperty = await apis.user.nodes.getSharedExpiryDate(
                    file6Id
                );
                ...
                expect(
                    Utils.formatDate(await shareDialog.getExpireDate())
                ).toEqual(Utils.formatDate(expiryDate));
            });
            ...
            it('Disable the share link expiration - [C286333]', async () => {
                ...
                expect(await shareDialog.getExpireDate()).not.toBe(
                    '',
                    'Expire date input is empty'
                );
                ...
                expect(await shareDialog.getExpireDate()).toBe(
                    '',
                    'Expire date input is not empty'
                );

                await shareDialog.clickClose();
                expect(await apis.user.nodes.getSharedExpiryDate(file7Id)).toBe(
                    '',
                    `${file7} link still has expiration`
                );
            });
            ...
            it('Share a file from the context menu - [C286345]', async () => {
                ...
                expect(await apis.user.nodes.isFileShared(file9Id)).toBe(
                    true,
                    `${file9} is not shared`
                );
                expect(url).toContain(sharedId);
        ...
        describe('from File Libraries', () => {
            ...
            beforeAll(async (done) => {
                ...
                parentInSiteId = (
                    await apis.user.nodes.createFolder(parentInSite, docLibId)
                ).entry.id;

                await apis.user.nodes.createFile(file1, parentInSiteId);
                await apis.user.nodes.createFile(file2, parentInSiteId);
                file3Id = (
                    await apis.user.nodes.createFile(file3, parentInSiteId)
                ).entry.id;
                await apis.user.nodes.createFile(file4, parentInSiteId);
                file5Id = (
                    await apis.user.nodes.createFile(file5, parentInSiteId)
                ).entry.id;
                file6Id = (
                    await apis.user.nodes.createFile(file6, parentInSiteId)
                ).entry.id;
                file7Id = (
                    await apis.user.nodes.createFile(file7, parentInSiteId)
                ).entry.id;
                await apis.user.nodes.createFile(file8, parentInSiteId);
                file9Id = (
                    await apis.user.nodes.createFile(file9, parentInSiteId)
                ).entry.id;
                await apis.user.shared.shareFileById(file6Id, expiryDate);
                await apis.user.shared.shareFileById(file7Id, expiryDate);
                await apis.user.shared.waitForApi({ expect: 2 });
        ...
        describe('from Shared Files', () => {
            ...
            it('Disable the share link expiration - [C286654]', async () => {
                ...
                await shareDialog.clickClose();
                expect(await apis.user.nodes.getSharedExpiryDate(file5Id)).toBe(
                    '',
                    `${file5} link still has expiration`
                );
            });
