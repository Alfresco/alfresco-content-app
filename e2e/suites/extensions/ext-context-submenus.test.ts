...
describe('Extensions - Context submenu', () => {
    ...
    beforeAll(async (done) => {
        ...
        await Utils.setSessionStorageFromConfig(
            EXTENSIBILITY_CONFIGS.CONTEXT_SUBMENUS
        );
    ...
    it('Displays the submenu actions set from config - [C286717]', async () => {
        ...
        expect(await contextMenu.hasSubMenu(menuItem1.label)).toBe(
            true,
            'Menu does not have submenu'
        );
        await contextMenu.mouseOverMenuItem(menuItem1.label);

        expect(await contextMenu.getSubmenuItemsCount()).toBe(
            3,
            'submenu has wrong number of items'
        );
        expect(
            await contextMenu.isSubMenuItemPresent(menuItem1.submenu[0])
        ).toBe(true, `${menuItem1.submenu[0]} is not displayed for ${file}`);
        expect(
            await contextMenu.isSubMenuItemPresent(menuItem1.submenu[1])
        ).toBe(true, `${menuItem1.submenu[1]} is not displayed for ${file}`);
        expect(
            await contextMenu.isSubMenuItemPresent(menuItem1.submenu[2])
        ).toBe(
            true,
