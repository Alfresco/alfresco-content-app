...
describe('File Libraries', () => {
    ...
    afterAll(async (done) => {
        await apis.user.sites.deleteSites([
            userSitePublic,
            userSiteModerated,
            userSitePrivate,
            siteId1,
    ...
    describe('My Libraries', () => {
        ...
        it('User can see only the sites he is a member of - [C280501]', async () => {
            ...
            expect(sitesCount).toEqual(
                10,
                'Incorrect number of sites displayed'
            );
            expect(await dataTable.isItemPresent(adminSite5)).toBe(
                false,
                `${adminSite5} should not appear in the list`
            );
        });
        ...
        it('Site ID is displayed when two sites have the same name - [C217098]', async () => {
            ...
            const actualSites = await dataTable.getCellsContainingName(
                siteName
            );
        ...
        it('Tooltip for sites with description - [C217097]', async () => {
            const tooltip = await dataTable.getItemNameTooltip(
                userSiteModerated
            );
    ...
    describe('Favorite Libraries', () => {
        ...
        it('User can see only his favorite sites - [C289897]', async () => {
            ...
            expect(await dataTable.isItemPresent(adminSite6)).toBe(
                false,
                `${adminSite6} should not appear`
            );
        });
