...
describe('Personal Files', () => {
    ...
    describe(`Admin user's personal files`, () => {
        ...
        it('has Data Dictionary and created content - [C213241]', async () => {
            expect(await dataTable.isItemPresent('Data Dictionary')).toBe(
                true,
                'Data Dictionary not displayed'
            );
            expect(await dataTable.isItemPresent(adminFolder)).toBe(
                true,
                'admin folder not displayed'
            );
        });
    });
    ...
    describe(`Regular user's personal files`, () => {
        ...
        it('has default sorted column - [C217143]', async () => {
            expect(await dataTable.getSortedColumnHeaderText()).toBe(
                'Modified'
            );
        });

        it('has user created content - [C213242]', async () => {
            expect(await dataTable.isItemPresent(userFolder)).toBe(
                true,
                'user folder not displayed'
            );
        });

        it('navigates to folder - [C213244]', async () => {
            const nodeId = (
                await apis.user.nodes.getNodeByPath(`/${userFolder}`)
            ).entry.id;
            ...
            expect(await browser.getCurrentUrl()).toContain(
                nodeId,
                'Node ID is not in the URL'
            );
            expect(await dataTable.isItemPresent(userFile)).toBe(
                true,
        ...
        it('redirects to Personal Files on clicking the link from sidebar - [C213245]', async () => {
            ...
            expect(url.endsWith(APP_ROUTES.PERSONAL_FILES)).toBe(
                true,
                'incorrect url'
            );
        });
