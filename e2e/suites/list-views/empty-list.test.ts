...
describe('Empty list views', () => {
    ...
    it('empty Personal Files - [C280131]', async () => {
        ...
        expect(await dataTable.getEmptyDragAndDropText()).toContain(
            'Drag and drop'
        );
    });
    ...
    it('empty My Libraries - [C217099]', async () => {
        ...
        expect(await dataTable.isEmpty()).toBe(true, 'list is not empty');
        expect(await dataTable.getEmptyStateTitle()).toContain(
    ...
    it('empty Trash - [C280134]', async () => {
        ...
        expect(await dataTable.getEmptyListText()).toContain(
    ...
    it('Favorites - pagination controls not displayed - [C280111]', async () => {
        await page.clickFavorites();
        expect(await pagination.isRangePresent()).toBe(
            false,
            'Range is present'
        );
        expect(await pagination.isMaxItemsPresent()).toBe(
            false,
            'Max items is present'
        );
        expect(await pagination.isCurrentPagePresent()).toBe(
            false,
            'Current page is present'
        );
        expect(await pagination.isTotalPagesPresent()).toBe(
            false,
            'Total pages is present'
        );
        expect(await pagination.isPreviousButtonPresent()).toBe(
            false,
            'Previous button is present'
        );
        expect(await pagination.isNextButtonPresent()).toBe(
            false,
    ...
    it('Search filters panel is not displayed on empty Search Results page - [C279189]', async () => {
        ...
        expect(
            await searchResultsPage.filters.isSearchFiltersPanelDisplayed()
        ).toBe(false, 'Search filters panel is present');
    });
    ...
    it('Empty Search results - Libraries - [C290020]', async () => {
        ...
        expect(await dataTable.isEmpty()).toBe(true, 'list is not empty');
        expect(await dataTable.getEmptySearchResultsText()).toContain(
