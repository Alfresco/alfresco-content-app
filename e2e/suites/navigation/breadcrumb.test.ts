...
describe('Breadcrumb', () => {
    ...
    beforeAll(async (done) => {
        ...
        subFolder1Id = (
            await apis.user.nodes.createFolder(subFolder1, parentId)
        ).entry.id;
        subFolder2Id = (
            await apis.user.nodes.createFolder(subFolder2, subFolder1Id)
        ).entry.id;
        await apis.user.nodes.createFile(fileName1, subFolder2Id);

        parent2Id = (await apis.user.nodes.createFolder(parent2)).entry.id;
        folder1Id = (await apis.user.nodes.createFolder(folder1, parent2Id))
            .entry.id;

        await apis.user.sites.createSite(siteName, SITE_VISIBILITY.PUBLIC);
        const docLibId = await apis.user.sites.getDocLibId(siteName);
        parentId = (await apis.user.nodes.createFolder(parent, docLibId)).entry
            .id;
        subFolder1Id = (
            await apis.user.nodes.createFolder(subFolder1, parentId)
        ).entry.id;
    ...
    it('Personal Files breadcrumb main node - [C260964]', async () => {
        ...
        expect(await breadcrumb.getItemsCount()).toEqual(
            1,
            'Breadcrumb has incorrect number of items'
        );
    ...
    it('Favorite Libraries breadcrumb main node - [C289891]', async () => {
        ...
        expect(await breadcrumb.getCurrentItemName()).toBe(
            'Favorite Libraries'
        );
    });
    ...
    describe('as admin', () => {
        ...
        beforeAll(async (done) => {
            ...
            userFolderId = (await user2Api.nodes.createFolder(userFolder)).entry
                .id;
