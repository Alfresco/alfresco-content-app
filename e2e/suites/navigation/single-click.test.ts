...
describe('Single click on item name', () => {
    ...
    beforeAll(async (done) => {
        ...
        deletedFile1Id = (await apis.user.nodes.createFile(deletedFile1)).entry
            .id;
        deletedFolder1Id = (await apis.user.nodes.createFolder(deletedFolder1))
            .entry.id;
    ...
    it('Hyperlink does not appear for items in the Trash - [C284899]', async () => {
        ...
        expect(await dataTable.hasLinkOnName(deletedFile1)).toBe(
            false,
            'Link on name is present'
        );
        expect(await dataTable.hasLinkOnName(deletedFolder1)).toBe(
            false,
            'Link on name is present'
        );
    });
    ...
    describe('on Personal Files', () => {
        ...
        it('Hyperlink appears when mouse over a file/folder - [C280032]', async () => {
            expect(await dataTable.hasLinkOnName(file1)).toBe(
                true,
                'Link on name is missing'
            );
        });
    ...
    describe('on File Libraries', () => {
        ...
        it('Hyperlink appears when mouse over a library - [C284901]', async () => {
            expect(await dataTable.hasLinkOnName(siteName)).toBe(
                true,
                'Link on site name is missing'
            );
        });
    ...
    describe('on Search Results', () => {
        ...
        it('Hyperlink appears when mouse over a file - [C306988]', async () => {
            ...
            expect(await dataTable.hasLinkOnSearchResultName(file1)).toBe(
                true,
                'Link on name is missing'
            );
        });
