...
describe('Pagination on multiple pages', () => {
    ...
    describe('on My Libraries', () => {
        ...
        it('Pagination control default values - [C280086]', async () => {
            ...
            expect(await pagination.isPreviousEnabled()).toBe(
                false,
                'Previous button is enabled'
            );
            expect(await pagination.isNextEnabled()).toBe(
                true,
        ...
        it('change the current page from menu - [C280089]', async () => {
            ...
            expect(await dataTable.isItemPresent('site-60')).toBe(
                true,
                'Site-60 not found on page'
            );
        ...
        it('navigate to next and previous pages - [C280092]', async () => {
            ...
            expect(await dataTable.isItemPresent('site-31')).toBe(
                true,
                'Site-31 not found on page'
            );
            await pagination.resetToDefaultPageNumber();
            ...
            await dataTable.waitForHeader();
            expect(await pagination.getRange()).toContain('1-25 of 101');
            expect(await dataTable.isItemPresent('site-12')).toBe(
                true,
                'Site-12 not found on page'
            );
        ...
        it('Previous button is disabled on first page - [C280090]', async () => {
            ...
            expect(await pagination.isPreviousEnabled()).toBe(
                false,
                'Previous button is enabled on first page'
            );
        });
        ...
        it('Next button is disabled on last page - [C280091]', async () => {
            ...
            expect(await dataTable.getRowsCount()).toBe(
                1,
                'Incorrect number of items on the last page'
            );
            expect(await pagination.getCurrentPage()).toContain('Page 5');
            expect(await pagination.isNextEnabled()).toBe(
                false,
                'Next button is enabled on last page'
            );
        });
    });
    ...
    describe('on Favorite Libraries', () => {
        ...
        it('change the current page from menu - [C291878]', async () => {
            ...
            expect(await dataTable.isItemPresent('site-40')).toBe(
                true,
                'Site-60 not found on page'
            );
        ...
        it('navigate to next and previous pages - [C291881]', async () => {
            ...
            await dataTable.waitForHeader();
            expect(await pagination.getRange()).toContain('26-50 of 101');
            expect(await dataTable.isItemPresent('site-70')).toBe(
                true,
                'Site-31 not found on page'
            );
            await pagination.resetToDefaultPageNumber();
            ...
            await dataTable.waitForHeader();
            expect(await pagination.getRange()).toContain('1-25 of 101');
            expect(await dataTable.isItemPresent('site-88')).toBe(
                true,
                'Site-12 not found on page'
            );
