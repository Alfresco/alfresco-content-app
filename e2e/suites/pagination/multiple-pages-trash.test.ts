...
describe('Pagination on multiple pages on Trash', () => {
    ...
    beforeAll(async () => {
        ...
        filesDeletedIds = (
            await userApi.nodes.createFiles(filesForDelete)
    ...
    it('change the current page from menu - [C280125]', async () => {
        ...
        expect(await dataTable.isItemPresent('file-40')).toBe(
            true,
    ...
    it('navigate to next and previous pages - [C280128]', async () => {
        ...
        expect(await dataTable.isItemPresent('file-70')).toBe(
            true,
            'File not found on page'
        );
        await pagination.resetToDefaultPageNumber();
        ...
        await dataTable.waitForHeader();
        expect(await pagination.getRange()).toContain('1-25 of 101');
        expect(await dataTable.isItemPresent('file-88')).toBe(
            true,
            'File not found on page'
        );
