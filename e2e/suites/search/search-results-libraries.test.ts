...
describe('Search results - libraries', () => {
    ...
    it('Search library - full name match - [C290012]', async () => {
        ...
        expect(await dataTable.isItemPresent(site1.name)).toBe(
            true,
            `${site1.name} not displayed`
        );
        expect(await dataTable.isItemPresent(site2.name)).toBe(
            false,
            `${site2.name} displayed`
        );
        expect(await dataTable.isItemPresent(site3.name)).toBe(
            false,
            `${site3.name} displayed`
        );
        expect(await dataTable.isItemPresent(site4.name)).toBe(
            false,
            `${site4.name} displayed`
        );
    });
    ...
    it('Private sites are not displayed when user is not a member - [C290019]', async () => {
        ...
        expect(await dataTable.isItemPresent(adminPrivate)).toBe(
            false,
            `${adminPrivate} is displayed`
        );
    });
    ...
    it('Search libraries with special characters - [C290028]', async () => {
        ...
        expect(await dataTable.isItemPresent(siteRussian.name)).toBe(
            true,
            `${siteRussian.name} not displayed`
        );
    });
});
