...
describe('Search filters', () => {
    ...
    beforeAll(async (done) => {
        ...
        await apis.user1.sites.addSiteMember(
            site,
            user2,
        ...
        await apis.user1.nodes.setGranularPermission(
            parentId,
            true,
            user2,
            'Collaborator'
        ...
        await apis.user1.upload.uploadFileWithRename(
            fileJpgUser1.source,
            docLibId,
            fileJpgUser1.name
        );
    ...
    it('Filters are displayed - [C279186]', async () => {
        expect(await sizeFilter.isPanelDisplayed()).toBe(
            true,
            'Size filter panel not displayed'
        );
        expect(await createdDateFilter.isPanelDisplayed()).toBe(
            true,
            'Created date filter panel not displayed'
        );
        expect(await fileTypeFilter.isPanelDisplayed()).toBe(
            true,
            'File type filter panel not displayed'
        );
        expect(await creatorFilter.isPanelDisplayed()).toBe(
            true,
            'Creator filter panel not displayed'
        );
        expect(await modifierFilter.isPanelDisplayed()).toBe(
            true,
            'Modifier filter panel not displayed'
        );
        expect(await locationFilter.isPanelDisplayed()).toBe(
            true,
            'Location filter panel not displayed'
        );
        expect(await modifiedDateFilter.isPanelDisplayed()).toBe(
            true,
            'Modified date filter panel not displayed'
        );
    });
    ...
    describe('Filter by Size', () => {
        ...
        it('Expand / Collapse the Size filter panel - [C279197]', async () => {
            expect(await sizeFilter.isPanelExpanded()).toBe(
                false,
                'Size filter panel is expanded'
            );

            await sizeFilter.expandPanel();
            expect(await sizeFilter.isPanelExpanded()).toBe(
                true,
                'Size filter panel not expanded'
            );

            const expectedSizes = ['Small', 'Medium', 'Large', 'Huge'];
            expect(await sizeFilter.getFiltersValues()).toEqual(
                expectedSizes,
                'Incorrect Size filters facets'
            );
            expect(await sizeFilter.isClearButtonEnabled()).toBe(
                true,
                'Size filter Clear button not enabled'
            );
        ...
        it('Filter by Small - [C279199]', async () => {
            ...
            expect(await dataTable.isItemPresent(fileJpgUser1.name)).toBe(
                true,
                `${fileJpgUser1.name} not in the list`
            );
            expect(await dataTable.isItemPresent(filePdfUser2.name)).toBe(
                true,
                `${filePdfUser2.name} not in the list`
            );
        });
        ...
        it('Clear the Size filter options - [C279198]', async () => {
            ...
            await sizeFilter.checkSizeSmall();
            await sizeFilter.checkSizeMedium();
            expect(await sizeFilter.getFiltersCheckedValues()).toEqual(
                ['Small', 'Medium'],
                'Incorrect checked Size filters'
            );

            await sizeFilter.clickClearButton();
            expect(await sizeFilter.getFiltersCheckedValues()).toEqual(
                [],
                'Size filters not cleared'
            );
        });
    });
    ...
    describe('Filter by Created date', () => {
        ...
        it('Expand / Collapse the Created date filter panel - [C279211]', async () => {
            expect(await createdDateFilter.isPanelExpanded()).toBe(
                false,
                'Created date filter panel is expanded'
            );

            await createdDateFilter.expandPanel();
            expect(await createdDateFilter.isPanelExpanded()).toBe(
                true,
                'Created date filter panel not expanded'
            );

            expect(await createdDateFilter.isClearButtonEnabled()).toBe(
                true,
                'Created date CLEAR button not enabled'
            );
            expect(await createdDateFilter.isApplyButtonEnabled()).toBe(
                false,
                'Created date APPLY button not disabled'
            );
        ...
        it('Results are filtered by Created date - [C279217]', async () => {
            ...
            expect(await createdDateFilter.isApplyButtonEnabled()).toBe(
                true,
                'Created date filter Apply button not enabled'
            );
            ...
            expect(await dataTable.isItemPresent(filePdfUser2.name)).toBe(
                true,
                'PDF file not displayed'
            );
            expect(await dataTable.isItemPresent(fileJpgUser1.name)).toBe(
                true,
                'JPG file not displayed'
            );
        });
        ...
        it('Clear the Created date filter options - [C279216]', async () => {
            ...
            expect(await createdDateFilter.getFromValue()).toEqual(
                '',
                'From field not empty'
            );
            expect(await createdDateFilter.getToValue()).toEqual(
                '',
                'To field not empty'
            );
        });
        ...
        it('From and To values are required - [C279212]', async () => {
            ...
            expect(await createdDateFilter.isFromErrorDisplayed()).toBe(
                true,
                'Error missing for From field'
            );
            expect(await createdDateFilter.isToErrorDisplayed()).toBe(
                true,
                'Error missing for To field'
            );
            expect(await createdDateFilter.getFromError()).toEqual(
                'Required value'
            );
            expect(await createdDateFilter.getToError()).toEqual(
                'Required value'
            );
        });
    ...
    describe('Filter by File type', () => {
        ...
        it('Expand / Collapse the File type filter panel - [C279191]', async () => {
            expect(await fileTypeFilter.isPanelExpanded()).toBe(
                true,
                'File type filter panel not expanded'
            );
            ...
            expect(await fileTypeFilter.isPanelExpanded()).toBe(
                false,
                'File type filter panel is expanded'
            );
        });
        ...
        it('Results are filtered by File type - [C279192]', async () => {
            ...
            expect(await fileTypeFilter.isClearButtonEnabled()).toBe(
                true,
                'File type filter Clear button not enabled'
            );
            expect(await dataTable.isItemPresent(filePdfUser2.name)).toBe(
                true,
                'PDF file not displayed'
        ...
        it('Clear the File type filter options - [C279193]', async () => {
            ...
            expect(await fileTypeFilter.getFiltersCheckedValues()).toEqual([
            ...
            expect(await fileTypeFilter.getFiltersCheckedValues()).toEqual(
                [],
                'File types selection not cleared'
            );
        });
    ...
    describe('Filter by Creator', () => {
        ...
        it('Expand / Collapse the Creator filter panel - [C279205]', async () => {
            expect(await creatorFilter.isPanelExpanded()).toBe(
                true,
                'Creator filter panel not expanded'
            );
            ...
            expect(await creatorFilter.isPanelExpanded()).toBe(
                false,
                'Creator filter panel is expanded'
            );
        });
        ...
        it('Results are filtered by Creator - [C279206]', async () => {
            ...
            expect(await creatorFilter.isClearButtonEnabled()).toBe(
                true,
                'Creator filter Clear button not enabled'
            );
            expect(await dataTable.isItemPresent(filePdfUser2.name)).toBe(
                false,
                'PDF file is displayed'
        ...
        it('Clear the Creator filter options - [C279207]', async () => {
            ...
            expect(await creatorFilter.getFiltersCheckedValues()).toEqual([
                `${user1} ${user1} (1)`,
            ]);
            expect(await dataTable.isItemPresent(filePdfUser2.name)).toBe(
                false,
    ...
    describe('Filter by Modifier', () => {
        ...
        it('Expand / Collapse the Modifier filter panel - [C279224]', async () => {
            expect(await modifierFilter.isPanelExpanded()).toBe(
                true,
                'Modifier filter panel not expanded'
            );
            ...
            expect(await modifierFilter.isPanelExpanded()).toBe(
                false,
                'Modifier filter panel is expanded'
            );
        });
        ...
        it('Results are filtered by Modifier - [C279225]', async () => {
            ...
            expect(await modifierFilter.isClearButtonEnabled()).toBe(
                true,
                'Modifier filter Clear button not enabled'
            );
            expect(await dataTable.isItemPresent(filePdfUser2.name)).toBe(
                false,
                'PDF file is displayed'
        ...
        it('Clear the Modifier filter options - [C279226]', async () => {
            ...
            expect(await modifierFilter.getFiltersCheckedValues()).toEqual([
                `${user1} ${user1} (1)`,
            ]);
            expect(await dataTable.isItemPresent(filePdfUser2.name)).toBe(
                false,
            ...
            expect(await modifierFilter.getFiltersCheckedValues()).toEqual(
                [],
                'Modifier selection not cleared'
            );
        });
    ...
    describe('Filter by Location', () => {
        ...
        it('Expand / Collapse the Location filter panel - [C279230]', async () => {
            expect(await locationFilter.isPanelExpanded()).toBe(
                true,
                'Location filter panel not expanded'
            );

            expect(await locationFilter.getFiltersValues()).toEqual(
                expectedLocations,
                'Incorrect Location filters facets'
            );
            ...
            expect(await locationFilter.isPanelExpanded()).toBe(
                false,
                'Location filter panel is expanded'
            );
        });
        ...
        it('Results are filtered by Location - [C279231]', async () => {
            ...
            expect(await locationFilter.isClearButtonEnabled()).toBe(
                true,
                'Location filter Clear button not enabled'
            );
            expect(await dataTable.isItemPresent(filePdfUser2.name)).toBe(
                false,
                'PDF file is displayed'
        ...
        it('Clear the Location filter options - [C279232]', async () => {
            ...
            expect(await locationFilter.getFiltersCheckedValues()).toEqual([
                `${site} (1)`,
            ]);
            expect(await dataTable.isItemPresent(filePdfUser2.name)).toBe(
                false,
            ...
            expect(await locationFilter.getFiltersCheckedValues()).toEqual(
                [],
                'Location selection not cleared'
            );
        });
    ...
    describe('Filter by Modified date', () => {
        ...
        it('Expand / Collapse the Modified date filter panel - [C279219]', async () => {
            expect(await modifiedDateFilter.isPanelExpanded()).toBe(
                true,
                'Modified Date filter panel not expanded'
            );
            ...
            expect(
                await modifiedDateFilter.isFilterCategoryInputDisplayed()
            ).toBe(true, 'Modified Date filter categories not displayed');

            await modifiedDateFilter.collapsePanel();
            expect(await modifiedDateFilter.isPanelExpanded()).toBe(
                false,
                'Modified Date filter panel is expanded'
            );
        });
        ...
        it('Clear the Modified date filter options - [C279220]', async () => {
            ...
            expect(await modifiedDateFilter.getFiltersCheckedValues()).toEqual(
                [],
                'Modified date selection not cleared'
            );
        });
    ...
    describe('Multiple filters', () => {
        ...
        it('Pagination is correct when search results are filtered - [C279188]', async () => {
            ...
            expect(await page.pagination.getRange()).toEqual(
                'Showing 1-1 of 1'
            );
