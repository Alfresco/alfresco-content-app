...
describe('File / Folder properties', () => {
    ...
    beforeAll(async (done) => {
        ...
        image1Id = (await apis.user.upload.uploadFile(image1.name, parentId))
            .entry.id;
    ...
    describe('View properties', () => {
        it('Default tabs - [C299162]', async () => {
            ...
            expect(await infoDrawer.isPropertiesTabDisplayed()).toBe(
                true,
                'Properties tab is not displayed'
            );
            expect(await infoDrawer.isCommentsTabDisplayed()).toBe(
                true,
                'Comments tab is not displayed'
            );
            expect(await infoDrawer.getTabsCount()).toBe(
                2,
        ...
        it('File properties - [C269003]', async () => {
            ...
            expect(await propertiesTab.isEditPropertiesButtonEnabled()).toBe(
                true,
                'Edit button not enabled'
            );
            expect(await propertiesTab.isMoreInfoButtonEnabled()).toBe(
                true,
                'More information button not enabled'
            );
        });
        ...
        it('Less / More information buttons - [C269004]', async () => {
            ...
            expect(await propertiesTab.isPropertiesListExpanded()).toBe(
                true,
                'Properties list not expanded'
            );

            await propertiesTab.clickMoreInformationButton();

            expect(await propertiesTab.isMoreInfoButtonDisplayed()).toBe(
                false,
                'More information button displayed'
            );
            expect(await propertiesTab.isLessInfoButtonEnabled()).toBe(
                true,
                'Less information button not enabled'
            );
            expect(await propertiesTab.isPropertiesListExpanded()).toBe(
                false,
            ...
            expect(await propertiesTab.isMoreInfoButtonDisplayed()).toBe(
                true,
                'More information button not displayed'
            );
            expect(await propertiesTab.isLessInfoButtonEnabled()).toBe(
                false,
                'Less information button enabled'
            );
            expect(await propertiesTab.isPropertiesListExpanded()).toBe(
                true,
                'Properties list not expanded'
        ...
        it('Image properties - [C269007]', async () => {
            ...
            const expectedPropValues = [
                ...
                moment(
                    apiProps.entry.properties['exif:dateTimeOriginal']
            ...
            expect(await propertiesTab.isImagePropertiesPanelDisplayed()).toBe(
                true,
                'Image properties panel not displayed'
            );
            expect(await propertiesTab.getVisiblePropertiesLabels()).toEqual(
                expectedPropLabels,
                'Incorrect properties displayed'
