...
describe('Library properties', () => {
    ...
    beforeAll(async (done) => {
        ...
        await apis.user.sites.addSiteMember(
            site.id,
            user2,
    ...
    it('Info drawer opens for a library - [C289336]', async () => {
        ...
        expect(await infoDrawer.isAboutTabDisplayed()).toBe(
            true,
            'About tab is not displayed'
        );
        expect(await aboutTab.isNameDisplayed()).toBe(
            true,
            'Name field not displayed'
        );
        expect(await aboutTab.isLibraryIdDisplayed()).toBe(
            true,
            'Library ID field not displayed'
        );
        expect(await aboutTab.isVisibilityDisplayed()).toBe(
            true,
            'Visibility field not displayed'
        );
        expect(await aboutTab.isDescriptionDisplayed()).toBe(
            true,
        ...
        expect((await aboutTab.getVisibility()).toLowerCase()).toEqual(
            site.visibility.toLowerCase()
        );
        expect(await aboutTab.getDescription()).toEqual(site.description);

        expect(await aboutTab.isEditLibraryPropertiesDisplayed()).toBe(
            true,
    ...
    it('Editable properties - [C289338]', async () => {
        ...
        expect(await aboutTab.isEditLibraryPropertiesEnabled()).toBe(
            true,
            'Edit action is not enabled'
        );
        await aboutTab.clickEditLibraryProperties();

        expect(await aboutTab.isNameEnabled()).toBe(
            true,
            'Name field not enabled'
        );
        expect(await aboutTab.isLibraryIdEnabled()).toBe(
            false,
            'Library ID field not disabled'
        );
        expect(await aboutTab.isVisibilityEnabled()).toBe(
            true,
            'Visibility field not enabled'
        );
        expect(await aboutTab.isDescriptionEnabled()).toBe(
            true,
            'Description field not enabled'
        );

        expect(await aboutTab.isCancelDisplayed()).toBe(
            true,
            'Cancel button not displayed'
        );
        expect(await aboutTab.isUpdateDisplayed()).toBe(
            true,
            'Update button not displayed'
        );
        expect(await aboutTab.isCancelEnabled()).toBe(
            true,
            'Cancel button not enabled'
        );
        expect(await aboutTab.isUpdateEnabled()).toBe(
            false,
    ...
    it('Edit site details - [C289339]', async () => {
        ...
        expect(await dataTable.isItemPresent(siteUpdated.name)).toBe(
            true,
            'New site name not displayed in the list'
        );
        expect(await infoDrawer.isOpen()).toBe(false, 'Info drawer still open');

        expect(
            (await apis.user.sites.getSite(siteForUpdate.id)).entry.title
        ).toEqual(siteUpdated.name);
        expect(
            (await apis.user.sites.getSite(siteForUpdate.id)).entry.description
        ).toEqual(siteUpdated.description);
        expect(
            (await apis.user.sites.getSite(siteForUpdate.id)).entry.visibility
        ).toEqual(siteUpdated.visibility);
    });
    ...
    it('Cancel editing a site - [C289340]', async () => {
        ...
        expect(await dataTable.isItemPresent(newName)).toBe(
            false,
            'New site name is displayed in the list'
        );
        expect(await dataTable.isItemPresent(site.name)).toBe(
            true,
            'Original site name not displayed in the list'
        );
    ...
    it('Warning appears when editing the name of the library by entering an existing name - [C289341]', async () => {
        ...
        expect(await aboutTab.isMessageDisplayed()).toBe(
            true,
            'Message not displayed'
        );
        expect(await aboutTab.getMessage()).toEqual(
    ...
    it('Site name too long - [C289342]', async () => {
        ...
        expect(await aboutTab.isErrorDisplayed()).toBe(
            true,
            'Message not displayed'
        );
        expect(await aboutTab.getError()).toEqual(
    ...
    describe('Non manager', () => {
        ...
        it('Error notification - [C289344]', async () => {
            ...
            await apis.user.sites.updateSiteMember(
                site.id,
                user3,
