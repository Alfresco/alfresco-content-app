...
describe('Comments', () => {
    ...
    beforeAll(async (done) => {
        ...
        file2SharedId = (
            await apis.user.nodes.createFile(file2Shared, parentId)
        ).entry.id;
        file2FavoritesId = (
            await apis.user.nodes.createFile(file2Favorites, parentId)
        ).entry.id;

        fileWith1CommentId = (
            await apis.user.nodes.createFile(fileWith1Comment, parentId)
        ).entry.id;
        fileWith2CommentsId = (
            await apis.user.nodes.createFile(fileWith2Comments, parentId)
        ).entry.id;

        comment1File2Entry = (
            await apis.user.comments.addComment(
                fileWith2CommentsId,
                'first comment'
            )
        ).entry;
    ...
    describe('from Personal Files', () => {
        ...
        it('Comments tab default fields - [C299173]', async () => {
            ...
            expect(await commentsTab.getCommentsTabHeaderText()).toBe(
                'Comments (0)'
            );
            expect(await commentsTab.isCommentTextAreaDisplayed()).toBe(
                true,
                'Comment field not present'
            );
            expect(await commentsTab.isAddCommentButtonEnabled()).toBe(
                false,
                'Add comment button not disabled'
            );
        });
        ...
        it('Comments are displayed ordered by created date in descending order - [C280583]', async () => {
            ...
            expect(await commentsTab.getNthCommentId(1)).toContain(
                comment2File2Entry.id
            );
            expect(await commentsTab.getNthCommentId(2)).toContain(
                comment1File2Entry.id
            );
        });
        ...
        it('Add button is enabled when typing in the comment field - [C280589]', async () => {
            ...
            expect(await commentsTab.isAddCommentButtonEnabled()).toBe(
                true,
                'Add comment button not enabled'
            );
        });
        ...
        it('Add a comment on a file - [C280590]', async () => {
            ...
            expect(await commentsTab.isCommentDisplayed()).toBe(
                true,
                `Comment not displayed`
            );
            expect(await commentsTab.getCommentText()).toBe(
                myComment,
    ...
    describe('Comment info display', () => {
        ...
        it('File from Personal files - [C280582]', async () => {
            ...
            expect(
                await commentsTab.isCommentDisplayed(commentFile1Entry.id)
            ).toBe(
                true,
                `Comment with id: ${commentFile1Entry.id} not displayed`
            );
            expect(await commentsTab.getCommentText(commentFile1Entry.id)).toBe(
                commentFile1Entry.content,
                'Incorrect comment text'
            );
            expect(
                await commentsTab.getCommentUserName(commentFile1Entry.id)
            ).toBe(`${username} ${username}`, 'Incorrect comment user');
            ...
            expect(
                await commentsTab.isCommentUserAvatarDisplayed(
                    commentFile1Entry.id
                )
            ).toBe(true, 'User avatar not displayed');
        });
