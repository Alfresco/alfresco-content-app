...
describe('Generic tests : ', () => {
    ...
    describe('single selection', () => {
        ...
        it('selected row is marked with a check circle icon - [C213134]', async () => {
            ...
            expect(await dataTable.hasCheckMarkIcon(file1)).toBe(
                true,
                'check mark missing'
            );
        });
        ...
        it('Row is marked with a check circle icon on direct right click - [C286252]', async () => {
            ...
            expect(await dataTable.hasCheckMarkIcon(file2)).toBe(
                true,
                'check mark missing'
            );
        });
        ...
        it('Context menu appears on direct right click on an item - [C286253]', async () => {
            ...
            expect(await dataTable.hasContextMenu()).toBe(
                true,
        ...
        it('Context menu appears correctly when right clicking on another item - [C284666]', async () => {
            ...
            expect(await dataTable.hasCheckMarkIcon(file2)).toBe(
                true,
                `${file2} is not selected`
            );
            expect(await dataTable.hasCheckMarkIcon(file1)).toBe(
                false,
                `${file1} is not selected`
            );
        });
    ...
    describe('Actions are not displayed when no item is selected', () => {
        it('on Personal Files - [C213120]', async () => {
            await page.clickPersonalFilesAndWait();
            expect(await toolbar.isEmpty()).toBe(
                true,
                `actions displayed though nothing selected`
            );
        });
        ...
        it('on Search Results - [C291815]', async () => {
            ...
            expect(await toolbar.getButtons()).toEqual([
    ...
    describe('multiple selection', () => {
        ...
        it('Context menu appears when right clicking on a single item while having multiple items selected - [C286269]', async () => {
            ...
            expect(await dataTable.hasContextMenu()).toBe(
                true,
                `Context menu is not displayed for ${file1}`
            );
            expect(await dataTable.getSelectedRowsCount()).toEqual(
                1,
                'incorrect number of selected rows'
            );
            expect(await contextMenu.isEditFolderPresent()).toBe(
                false,
                `Edit folder is displayed for ${file1}`
            );
            expect(await dataTable.hasCheckMarkIcon(file1)).toBe(
                true,
                `${file1} is not selected`
            ...
            expect(await dataTable.hasCheckMarkIcon(folder1)).toBe(
                false,
                `${folder1} is selected`
            );
        });
