...
describe('Special permissions : ', () => {
    ...
    const userCollaboratorApi = new RepoClient(
        userCollaborator,
        userCollaborator
    );
    ...
    beforeAll(async () => {
        ...
        await userManagerApi.sites.addSiteCollaborator(
            sitePrivate,
            userCollaborator
        );
        await userManagerApi.sites.addSiteManager(sitePrivate, userDemoted);

        await userManagerApi.upload.uploadFileWithRename(
            FILES.docxFile,
            docLibId,
        ...
        fileFavId = (
            await userManagerApi.nodes.createFile(
        ...
        await userManagerApi.sites.updateSiteMember(
            sitePrivate,
            userDemoted,
        ...
        folderFavId = (
            await userManagerApi.nodes.createFolder(
        ...
        folderFav2Id = (
            await userManagerApi.nodes.createFolder(
                testData.folderFav2.name,
                docLibId
            )
        ).entry.id;
        await userConsumerApi.favorites.addFavoritesByIds('folder', [
            folderFavId,
            folderFav2Id,
        ]);
        ...
        await userCollaboratorApi.favorites.addFavoritesByIds('file', [
            fileDocxSharedFavId,
            fileSharedFavId,
        ]);
        ...
        await userManagerApi.favorites.addFavoriteById(
            'file',
            fileLockedByUserId
        );
