...
describe('Files / folders actions : ', () => {
    ...
    beforeAll(async () => {
        ...
        await userApi.upload.uploadFileWithRename(
            FILES.docxFile,
            parentId,
        ...
        fileFavId = (
            await userApi.nodes.createFile(testData.fileFav.name, parentId)
        ).entry.id;
        fileDocxSharedId = (
            await userApi.upload.uploadFileWithRename(
                FILES.docxFile,
        ...
        fileSharedId = (
            await userApi.nodes.createFile(testData.fileShared.name, parentId)
        ).entry.id;
        fileSharedFavId = (
            await userApi.nodes.createFile(
                testData.fileSharedFav.name,
                parentId
            )
        ).entry.id;
        fileLockedId = (
            await userApi.nodes.createFile(testData.fileLocked.name, parentId)
        ).entry.id;
        fileFavLockedId = (
            await userApi.nodes.createFile(
        ...
        folderFavId = (
            await userApi.nodes.createFolder(testData.folderFav.name, parentId)
        ).entry.id;
        folderFav2Id = (
            await userApi.nodes.createFolder(testData.folderFav2.name, parentId)
        ).entry.id;

        await userApi.favorites.addFavoritesByIds('folder', [
            folderFavId,
            folderFav2Id,
        ]);
        ...
        fileInTrashId = (
            await userApi.nodes.createFile(testData.fileInTrash.name)
        ).entry.id;
        file2InTrashId = (
            await userApi.nodes.createFile(testData.file2InTrash.name)
        ).entry.id;
        folderInTrashId = (
            await userApi.nodes.createFolder(testData.folderInTrash.name)
        ).entry.id;
        folder2InTrashId = (
            await userApi.nodes.createFolder(testData.folder2InTrash.name)
        ).entry.id;
