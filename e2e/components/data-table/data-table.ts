...
export class DataTable extends Component {
    ...
    emptySearchText: ElementFinder = this.component.element(
        by.css(DataTable.selectors.emptySearchText)
    );
    ...
    async waitForRowToBeSelected(): Promise<void> {
        await browser.wait(
            EC.presenceOf(
                this.component.element(by.css(DataTable.selectors.selectedRow))
            ),
            BROWSER_WAIT_TIMEOUT,
            'timeout waiting for row to be selected'
        );
    }
    ...
    async getSelectedRowsNames(): Promise<string[]> {
        const rowsText: string[] = await this.getSelectedRows().map((row) => {
            return row
                .element(by.css('.adf-datatable-cell[title="Name"]'))
                .getText();
        });
        return rowsText;
    }
    ...
    async getItemLocationTooltip(name: string): Promise<string> {
        ...
        const condition = () =>
            location
                .getAttribute('title')
                .then((value) => value && value.length > 0);
    ...
    async getEmptyListText(): Promise<string> {
        ...
        if (isEmpty) {
            return this.component
                .element(by.css('adf-custom-empty-content-template'))
                .getText();
        }
        return '';
    }
    ...
    async getCellsContainingName(name: string): Promise<string[]> {
        const rows: ElementArrayFinder = this.getRows().all(
    ...
    getSearchResultsRowByName(
        ...
        if (location) {
            return this.body
                .all(
                    by.cssContainingText(
                        DataTable.selectors.searchResultsRow,
                        name
                    )
                )
                .filter(async (elem) =>
    ...
    getSearchResultNameLink(
        itemName: string,
        location: string = ''
    ): ElementFinder {
        return this.getSearchResultsRowByName(itemName, location).$(
            DataTable.selectors.searchResultsNameLink
        );
    }
