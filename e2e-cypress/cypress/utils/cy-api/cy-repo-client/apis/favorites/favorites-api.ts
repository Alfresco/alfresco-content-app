...
import { CyRepoClient } from './../../cy-repo-client';
...
export class FavoritesApi extends RepoApi {
  ...
  async addFavorite(api: CyRepoClient, nodeType: string, name: string) {
    try {
  ...
  async isFavoriteWithRetry(nodeId: string, data: { expect: boolean }) {
    ...
    try {
      ...
      return await CyUtils.retryCall(favorite);
    } catch (error) {}
    return isFavorite;
  }
  ...
  async waitForApi(data: { expect: number }) {
    try {
      ...
      return await CyUtils.retryCall(favoriteFiles);
    } catch (error) {
      cy.log(`FavoritesApi waitForApi :  catch : `);
