...
export class SharedLinksApi extends RepoApi {
  ...
  async waitForApi(data: { expect: number }) {
    try {
      ...
      return await CyUtils.retryCall(sharedFiles);
    } catch (error) {
      cy.log(`SharedLinksApi waitForApi :  catch : `);
      cy.log(`\tExpected: ${data.expect} items, but found ${error}`);
    }
  }
  ...
  async waitForFilesToBeShared(filesIds: string[]) {
    try {
      ...
      return await CyUtils.retryCall(sharedFile);
    } catch (error) {
      cy.log(`SharedLinksApi waitForFilesToBeShared :  catch : `);
      cy.log(`\tWait timeout reached waiting for files to be shared`);
    }
  }
  ...
  async waitForFilesToNotBeShared(filesIds: string[]) {
    try {
      ...
      cy.log(`SharedLinksApi waitForFilesToNotBeShared :  catch : `);
      cy.log(`\tWait timeout reached waiting for files to no longer be shared`);
    }
  }
}
