...
import { SITE_VISIBILITY, SITE_ROLES } from '../../../../cy-configs';
...
export class SitesApi extends RepoApi {
  ...
  async waitForSitesToBeCreated(sitesIds: string[]) {
    try {
      ...
      return await CyUtils.retryCall(site);
    } catch (error) {
      cy.log(`SitesApi waitForSitesToBeCreated :  catch : `);
      cy.log(`\tWait timeout reached waiting for sites to be created`);
    }
  }
  ...
  async waitForApi(data: { expect: number }) {
    try {
      ...
      return await CyUtils.retryCall(sites);
    } catch (error) {
      cy.log(`SitesApi waitForApi : catch : `);
