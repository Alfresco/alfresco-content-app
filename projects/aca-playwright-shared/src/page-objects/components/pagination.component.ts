...
export class PaginationComponent extends BaseComponent {
  ...
  async spinnerWaitForReload(): Promise<void> {
    try {
      await this.getChild('mat-progress-spinner').waitFor({ state: 'attached', timeout: 2000 });
      await this.getChild('mat-progress-spinner').waitFor({ state: 'detached', timeout: 2000 });
    } catch (e) {
