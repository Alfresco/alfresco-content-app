...
describe('NodePermissionService', () => {
    ...
    describe('Multiple source permission', () => {
        ...
        it('should return true when source has `AND` allowableOperations permission', () => {
            const source = [
                {
                    entry: {
                        allowableOperations: ['update', 'delete', 'other'],
                    },
                },
                {
                    entry: {
                        allowableOperations: ['update', 'create', 'other'],
                    },
                },
                {
                    entry: {
            ...
            expect(
                permission.check(source, ['update', 'other'], {
                    operation: 'AND',
                })
            ).toBe(true);
        });
        ...
        it('should return false when source has no `AND` allowableOperations permission', () => {
            ...
            expect(
                permission.check(source, ['update', 'bogus'], {
                    operation: 'AND',
                })
            ).toBe(false);
        });
    ...
    describe('Single source permission', () => {
        ...
        it('should return true when source has `AND` allowableOperations permission', () => {
            const source = {
                entry: { allowableOperations: ['update', 'other'] },
            };
